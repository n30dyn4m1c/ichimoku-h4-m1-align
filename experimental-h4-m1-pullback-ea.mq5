//+------------------------------------------------------------------+
//| Experimental: H4-M1 Alignment EA with Kijun Pullback Entry       |
//| Idea:  a full 6-TF aligned breakout almost always retraces to     |
//|        the H4 Kijun (96.8% of episodes, median 1.3 days) and then |
//|        continues (91.9% make a new high). Instead of waiting for  |
//|        a full 6-TF re-alignment to re-enter, buy the pullback:    |
//|        trade WITH the still-aligned higher timeframes once a      |
//|        closed H4 bar has touched the H4 Kijun and bounced.        |
//|        Empirical study (GC=F 1h, 2y, H4 anchor): bounce-candle    |
//|        close back above the Kijun lifts the win rate from 26.5%   |
//|        to 61.6%; breakout extension >= 2 ATR: 60.5%; ADX(H4)>=30: |
//|        53.4%. Fading the breakout back to the Kijun won 0/13.     |
//| Entry: BREAKOUT mode = original 6-TF alignment (same as main EA). |
//|        PULLBACK mode = H4 Kijun bounce after a prior alignment.   |
//| Exit:  same as main H4-M1 EA: ATR chandelier trail, M15 Kijun     |
//|        cross fallback, ATR-based protective stop loss.            |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
//--- Entry mode: original breakout-only, pullback-only, or both
enum ENUM_ENTRY_MODE { ENTRY_BREAKOUT = 0, ENTRY_PULLBACK = 1, ENTRY_BOTH = 2 };

input group  "Pullback Entry"
input ENUM_ENTRY_MODE InpEntryMode       = ENTRY_PULLBACK; // 0=breakout only, 1=pullback only, 2=both
input int             InpPullTrendTFs    = 2;    // Top-N timeframes that must stay aligned (2=H4+H1, 3=+M30, ...)
input int             InpBounceLookbackH4 = 3;   // H4 bars back to find the Kijun touch
input bool            InpTouchCloseAbove = true; // Touch bar must close on the trend side of the H4 Kijun
input double          InpMaxEntryDistATR = 1.5;  // Max |price - H4 Kijun| for a pullback entry (x ATR H4)
input double          InpMinBreakoutExtATR = 2.0;// Prior breakout must be >= this x ATR(H4) extended (0=off)
input double          InpMinADX          = 30.0; // Min ADX(H4) for a pullback entry (0=off)
...
int      atrH4[MAX_SYMS]; // ATR(H4) handle for breakout-extension measurement
int      adxH4[MAX_SYMS]; // ADX(H4) handle for the pullback trend-strength filter
...
int      wasAligned[MAX_SYMS];   // direction of the last full 6-TF alignment signal (0 = none)
double   breakoutExt[MAX_SYMS];  // H4 extension of that signal, in ATR(H4) (always positive)
...
//==============================================================
// Pullback Entry Check: a prior full 6-TF alignment fired, the
// higher timeframes are still aligned, price retraced to the H4
// Kijun and a closed H4 bar bounced off it.
// Returns 1 (bullish), -1 (bearish), 0 (no setup)
//==============================================================

int CheckPullback(int s)
{
   int dir = wasAligned[s];
   if(dir == 0) return 0;

   // 1. Higher-TF trend intact: top InpPullTrendTFs timeframes still aligned
   int trendTFs = InpPullTrendTFs;
   if(trendTFs < 2)  trendTFs = 2;
   if(trendTFs > TF_COUNT) trendTFs = TF_COUNT;
   for(int t = 0; t < trendTFs; t++)
   {
      if(CheckAlign(s, t) != dir) return 0;
   }

   // 2. A closed H4 bar touched the H4 Kijun within the lookback window
   int look = MathMax(1, InpBounceLookbackH4);
   double kij[];
   if(CopyBuffer(ich[s][0], 1, 1, look, kij) <= 0) return 0;
   ArraySetAsSeries(kij, true);

   MqlRates h4[];
   if(CopyRates(syms[s], PERIOD_H4, 1, look, h4) <= 0) return 0;
   ArraySetAsSeries(h4, true);

   int touchBar = -1;   // most recent bar whose range contains the Kijun
   for(int i = 0; i < look; i++)
   {
      if(h4[i].low <= kij[i] && h4[i].high >= kij[i]) { touchBar = i; break; }
   }
   if(touchBar < 0) return 0;

   // 3. Bounce confirmation: the touch bar closed on the trend side of the Kijun
   if(InpTouchCloseAbove)
   {
      if(dir ==  1 && h4[touchBar].close <= kij[touchBar]) return 0;
      if(dir == -1 && h4[touchBar].close >= kij[touchBar]) return 0;
   }

   // 4. Current price still on the trend side, and close enough to the Kijun
   double kijNow[1];
   if(CopyBuffer(ich[s][0], 1, 1, 1, kijNow) <= 0) return 0;

   double px = (dir == 1) ? SymbolInfoDouble(syms[s], SYMBOL_ASK)
                          : SymbolInfoDouble(syms[s], SYMBOL_BID);
   if(dir ==  1 && px <= kijNow[0]) return 0;
   if(dir == -1 && px >= kijNow[0]) return 0;

   if(InpMaxEntryDistATR > 0)
   {
      double a4[1];
      if(CopyBuffer(atrH4[s], 0, 1, 1, a4) <= 0 || a4[0] <= 0) return 0;
      if(MathAbs(px - kijNow[0]) > InpMaxEntryDistATR * a4[0]) return 0;
   }

   // 5. Optional filters from the empirical study
   if(InpMinBreakoutExtATR > 0 && breakoutExt[s] < InpMinBreakoutExtATR) return 0;

   if(InpMinADX > 0)
   {
      double d[1];
      if(CopyBuffer(adxH4[s], 0, 1, 1, d) <= 0 || d[0] <= 0) return 0;
      if(d[0] < InpMinADX) return 0;
   }
...
int OpenPositions(int s, bool isBuy, double dist, int count, double lots,
                  string comment, double &firstFill)
{
   string sym = syms[s];
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, 0, comment)
...
void OnTick()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      // Entry check: no open position, cooldown elapsed, spread sane
      if(state[s] == 0 && TimeCurrent() >= noReentryUntil[s] && SpreadOK(syms[s]))
      {
         // Refresh the "prior breakout" memory while flat: any full 6-TF
         // alignment signal is the breakout this EA's pullback entries need.
         int al = CheckAllAlign(s);
         if(al != 0 && atrH4[s] != INVALID_HANDLE)
         {
            wasAligned[s] = al;
            double k[1], a4[1];
            MqlRates h4r[];
            if(CopyBuffer(ich[s][0], 1, 1, 1, k) <= 0) continue;
            if(CopyRates(syms[s], PERIOD_H4, 1, 1, h4r) <= 0) continue;
            if(CopyBuffer(atrH4[s], 0, 1, 1, a4) <= 0 || a4[0] <= 0) continue;
            breakoutExt[s] = (al == 1) ? (h4r[0].close - k[0]) / a4[0]
                                       : (k[0] - h4r[0].close) / a4[0];
         }
         ...
         {
            st = CheckAllAlign(s);
            if(st != 0) entryLabel = "Breakout";
         }
         if(st == 0 && InpEntryMode != ENTRY_BREAKOUT)
         {
            st = CheckPullback(s);
            if(st != 0) entryLabel = "Pullback";
         }
         ...
         if(st != 0)
         {
            ...
            string comment = isBuy ? "Buy H4-M1 " + entryLabel : "Sell H4-M1 " + entryLabel;
            double firstFill = 0.0;
            int filled = OpenPositions(s, isBuy, dist, count, lots, comment, firstFill);
            if(filled > 0)
            {
               state[s] = st;
               wasAligned[s] = 0;   // fresh breakout required for the next pullback
               ...
               Print(PCTime() + " | " + syms[s] + " " + entryLabel + " entry signal but no order filled");
         }
      }
   }
}
