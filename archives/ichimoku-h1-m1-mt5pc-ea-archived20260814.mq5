//+------------------------------------------------------------------+
//| Ichimoku H1-M1 Alignment EA (BE30) — MT5 desktop build           |
...
//| Desktop: copy of the VPS build for local MT5 use — Alert popups |
//|        and the weekly equity-withdrawal alert are restored; all  |
//|        logic still runs only on closed M1 bars (once per minute) |
...
datetime lastEquityBar = 0;         // H1 bar gating for the equity alert
...
int MAGIC = 20260831;   // fresh number — MT5 desktop build (20260814 is the live H1 VPS)
...
void OnTick()
{
   ...
   MqlRates h1eq[];
   if(symsCount > 0 && CopyRates(syms[0], PERIOD_H1, 0, 1, h1eq) > 0 && h1eq[0].time != lastEquityBar)
   {
      lastEquityBar = h1eq[0].time;
      CheckEquityAlert();
   }
