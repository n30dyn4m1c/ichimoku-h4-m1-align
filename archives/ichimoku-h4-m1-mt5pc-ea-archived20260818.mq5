//+------------------------------------------------------------------+
//| Ichimoku Alignment EA (BE30) — MT5 desktop build (merged dual)   |
//| Entry: InpTopTF = H4 -> H4→M1 price+chikou all above/below        |
//|        tenkan,kijun,cloud (was the live H4-M1 desktop build)      |
//|        InpTopTF = H1 -> H1→M1 price+chikou all above/below        |
//|        tenkan,kijun,cloud (was the live H1-M1 desktop build)      |
//| This single file replaces BOTH former desktop builds:            |
//|        ichimoku-h4-m1-mt5pc-ea.mq5 and ichimoku-h1-m1-mt5pc-ea.  |
//|        mq5 (the archived 2026-08-14 originals live in archives/)  |
//| Entry filters (experimental additions to the shared logic):      |
...
//| Desktop: copy of the VPS dual-mode build for local MT5 use —     |
//|        Alert popups and the weekly equity-withdrawal alert are   |
//|        restored; all logic still runs only on closed M1 bars     |
//|        (once per minute)                                         |
...
int MAGIC = 20260830;   // per mode: 20260830 = H4 stack, 20260831 = H1 stack
...
{
   SetupTimeframes();
   MAGIC = (InpTopTF == TOP_H4) ? 20260830 : 20260831;
...
void OnTick()
{
   // All logic runs only on closed M1 bars, which change at most
