//+------------------------------------------------------------------+
//| Ichimoku H4-H1 Alignment EA — swing build (experimental, BTC)   |
...
input double InpATRMultiplier     = 3.5;    // SL distance = ATR * multiplier (BTC wicks wider)
input int    InpMaxSpreadPoints   = 0;      // Max spread in points to allow entry (0 = no limit, BTC point spread is huge)
...
input double          InpTrailATR         = 4.0;   // Trail distance = ATR(H1) * multiplier (BTC wicks)
input double          InpTrailActivateATR = 2.5;   // Arm the trail once profit >= ATR(H1) * multiplier
...
int MAGIC = 20260819;   // fresh number — distinct from all D1-H4 / H4-M1 / H1-M1 builds
