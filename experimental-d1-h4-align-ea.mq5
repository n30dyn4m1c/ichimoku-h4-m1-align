...
//|        the peak, distance = ATR(H4) x multiplier), H4 close       |
//|        crosses H4 kijun as final fallback, or ATR-based           |
...
//|        when the D1/H4 move count (bars since the last Kijun       |
...
input double          InpTrailATR         = 3.0;   // Trail distance = ATR(H4) * multiplier
input double          InpTrailActivateATR = 2.0;   // Arm the trail once profit >= ATR(H4) * multiplier
...
input double InpBE30ActivateATR    = 0.5;    // Min profit to arm BE (x ATR H4)
...
#define MAX_SYMS  60
#define TF_COUNT  3
#define IDX_H1    2   // index of H1 in tfs[] — used for time-filter check
...
int MAGIC = 20260816;   // fresh number — distinct from all H4-M1 / H1-M1 builds
...
int OnInit()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      atrExit[s] = iATR(syms[s], PERIOD_H4, InpATRPeriod);
...
//==============================================================
// Entry Check: D1 and H4 aligned same direction (H1 not required —
// it is only a timing input for the bar-close cadence)
//==============================================================

int CheckAllAlign(int s)
{
   int dir = CheckAlign(s, 0);   // D1
   if(dir == 0) return 0;

   if(CheckAlign(s, 1) != dir) return 0;   // H4
   return dir;
}
...
// checks D1 first, then H4, then H1 — each must be clear
...
void LogTimeSkip(int s, int tfIdx, int count, int fracCount, string tfName)
{
   int key = tfIdx * 100000 + count * 100 + fracCount;
   if(lastSkipKey[s] == key) return;
   lastSkipKey[s] = key;
   Print(PCTime() + " | " + syms[s] + " skip entry: " + tfName + " time cycle mature (kijun count " + IntegerToString(count) + ", fractal count " + IntegerToString(fracCount) + ")");
}

// Nested kihon suchi cascade: D1 -> H4 -> H1. Each timeframe
...
bool TimeFilterBlocks(int s, int st)
{
   ...
   if(InpTimeFilterD1)
   {
      int cD = CountNoTouchTF(s, 0, st, KIHON_MAX_CYCLE);
      int cFD = CountSinceFractal(s, 0, st, KIHON_MAX_CYCLE);
      if(InTimeWindow(cD, KIHON_MAX_CYCLE) || InTimeWindow(cFD, KIHON_MAX_CYCLE))
      {
         LogTimeSkip(s, 0, cD, cFD, "D1");
         return true;
      }
   }
   if(InpTimeFilterH4)
   {
      int c4 = CountNoTouchTF(s, 1, st, KIHON_MAX_CYCLE);
      int cF4 = CountSinceFractal(s, 1, st, KIHON_MAX_CYCLE);
      if(InTimeWindow(c4, KIHON_MAX_CYCLE) || InTimeWindow(cF4, KIHON_MAX_CYCLE))
      {
         LogTimeSkip(s, 1, c4, cF4, "H4");
         return true;
      }
   }
   if(InpTimeFilterH1)
   {
      ...
      int cF1 = CountSinceFractal(s, IDX_H1, st, KIHON_MAX_CYCLE);
      if(InTimeWindow(c1, KIHON_MAX_CYCLE) || InTimeWindow(cF1, KIHON_MAX_CYCLE))
      {
         LogTimeSkip(s, IDX_H1, c1, cF1, "H1");
         return true;
      }
   }
   return false;
...
//==============================================================
// Exit Check: H4 price closed on wrong side of H4 kijun
//==============================================================

bool CheckH4Exit(int s, int dir)
{
   double kij[1];
   if(CopyBuffer(ich[s][1], 1, 1, 1, kij) <= 0) return false;

   MqlRates rt[];
   if(CopyRates(syms[s], PERIOD_H4, 1, 1, rt) <= 0) return false;
...
//==============================================================
// Exit Trail: wide ATR chandelier stop once the trade is in profit.
// The reference point is the extreme (high/low) of the H1 bar
// that is still forming, so a peak is locked in before it
// retraces; the distance and arm threshold use ATR(H4) so the
// trail survives H1 pullbacks and holds swings. Re-evaluated on
// every new H1 bar; only ever tightens and never sits inside the
// broker minimum stop.
//==============================================================
...
int OpenPositions(int s, bool isBuy, double dist, int count, double lots, double &firstFill)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, 0, "Buy D1-H4")
                      : trade.Sell(lots, sym, price, sl, 0, "Sell D1-H4");
...
void OnTick()
{
   for(int s = 0; s < symsCount; s++)
   {
      ...
      // Exit check: close all when H4 closes against direction across H4 kijun
      if(state[s] != 0 && CheckH4Exit(s, state[s]))
      {
         string side = (state[s] == 1) ? "Long" : "Short";
         string msg  = PCTime() + " | Close " + syms[s] + " " + side + " (H4 kijun crossed)";
      ...
      // Entry check: D1 and H4 must align, spread must be sane
      if(state[s] == 0 && TimeCurrent() >= noReentryUntil[s] && SpreadOK(syms[s]))
      {
         int st = CheckAllAlign(s);
         if(st != 0)
         {
            // Time theory: nested kihon suchi cascade — D1 must be
            // clear (count not exactly on a cycle) before H4 is
            // checked, then H1 (off by default). A mature count
            // anywhere in the chain blocks the entry.
