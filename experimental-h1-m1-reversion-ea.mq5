...
input string InpTimeCycles    = "9,17,26,33,42,51,65,76,83,97,101"; // Ichimoku kihon suchi cycles: bars since last H1 Kijun touch (touch candle = bar 1, present candle included)
...
// Most-recent H1 bars whose range does NOT contain the Kijun (i.e. the
// candle never touched the Kijun), counted from the first clear candle
// up to the present candle; the touching candle is candle 1. Stops at
// the first touch.
...
int CheckReversion(int s, double &sl, double &tp, double &stopDist, string &trig, int &barsSince)
{
   ...
   // must land on an Ichimoku cycle (9 / 17 / 26 / 33 / 42 / 51 / 65 / 76 / 83 / 97 / 101 by default) +/- InpTimeTol.
