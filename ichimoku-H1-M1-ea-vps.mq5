//+------------------------------------------------------------------+
//| Ichimoku H1-M1 Alignment EA — VPS build                          |
...
input int    InpMagic     = 20260802;  // Magic number (use 20260502 to adopt the non-VPS EA's positions)
...
#define MAX_SYMS  60
#define TF_COUNT  5
#define IDX_EXIT  3            // index of M5 in tfs[] — used for exit check
#define TF_EXIT   PERIOD_M5    // exit timeframe (must match tfs[IDX_EXIT])
...
int CheckAlign(int s, int tfIdx)
{
   ...
   // Without this, every M1 bar re-derives an H1 reading that only changes
   // once every 60 M1 bars.
...
void OnTick()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      if(state[s] != 0 && CheckExit(s, state[s]))
      {
         ...
         Notify(PCTime() + " | Close " + syms[s] + " " + side + " (M5 kijun crossed)");
