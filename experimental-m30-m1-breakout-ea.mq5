//+------------------------------------------------------------------+
//| Ichimoku M30-M1 Breakout Alignment EA                            |
//| Entry: M30→M1 price+chikou all above/below tenkan,kijun,cloud   |
...
#define MAX_SYMS  60
#define TF_COUNT  4
#define IDX_M5    2   // index of M5 in tfs[] — used for exit check
...
void SyncStateFromPositions()
{
   ...
   for(int s = 0; s < symsCount; s++) { state[s] = 0; hasPos[s] = false; }
...
//==============================================================
// Entry Check: all timeframes (M30→M1) aligned same direction
//==============================================================
...
bool GetExitATR(int s, double &val)
{
   val = 0.0;
   if(atrExit[s] == INVALID_HANDLE) return false;
   double a[1];
   if(CopyBuffer(atrExit[s], 0, 1, 1, a) <= 0 || a[0] <= 0) return false;
   val = a[0];
   return true;
}
...
int OpenPositions(int s, bool isBuy, double dist, int count, double lots, double &firstFill)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, 0, "Buy M30-M1")
                      : trade.Sell(lots, sym, price, sl, 0, "Sell M30-M1");
...
void OnTick()
{
   // Equity alert: fire only on a new M30 bar (CheckEquityAlert self-guards on day-of-week)
   MqlRates m30[];
   if(symsCount > 0 && CopyRates(syms[0], PERIOD_M30, 0, 1, m30) > 0 && m30[0].time != lastAlertBar)
   {
      lastAlertBar = m30[0].time;
      CheckEquityAlert();
   }
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      // Entry check: all timeframes M30→M1 must align, spread must be sane
