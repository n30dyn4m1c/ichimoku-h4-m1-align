//+------------------------------------------------------------------+
//| Ichimoku H4-M15 Alignment EA (experimental)                      |
//| Entry: H4→M15 price+chikou all above/below tenkan,kijun,cloud    |
//|        (H4, H1, M30, M15 — M5 and M1 disregarded)                |
...
#define MAX_SYMS  60
#define TF_COUNT  4
#define IDX_M15   3   // index of M15 in tfs[] — used for exit check and bar gating

ENUM_TIMEFRAMES tfs[TF_COUNT] = {
   PERIOD_H4, PERIOD_H1, PERIOD_M30, PERIOD_M15
};
...
//==============================================================
// Entry Check: all timeframes (H4→M15) aligned same direction
//==============================================================
...
int OpenPositions(int s, bool isBuy)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, 0, "Buy H4-M15")
                      : trade.Sell(lots, sym, price, sl, 0, "Sell H4-M15");
...
void OnTick()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      // Entry check: all timeframes H4→M15 must align, spread must be sane
      if(state[s] == 0 && SpreadOK(syms[s]))
      {
         ...
         if(st != 0)
         {
            ...
            string msg = PCTime() + " | " + action + " " + syms[s] +
                         ...
                         " @ " + DoubleToString(msgLots, 2) + " (H4-M15)";
            Print(msg); Alert(msg); SendNotification(msg);
