//+------------------------------------------------------------------+
//| Ichimoku H4-H1 Alignment EA — swing build (experimental, US30)  |
...
input int    InpMaxSpreadPoints   = 100;    // Max spread in points to allow entry (0 = no limit)
...
input double          InpTrailATR         = 2.5;   // Trail distance = ATR(H1) * multiplier (smooth index)
input double          InpTrailActivateATR = 1.5;   // Arm the trail once profit >= ATR(H1) * multiplier
...
int MAGIC = 20260820;   // fresh number — distinct from all D1-H4 / H4-M1 / H1-M1 builds
