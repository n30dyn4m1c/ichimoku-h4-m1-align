...
| **Kihon-Suchi + PO3 Gate EA** (`experimental-bottomup-stack-kihon-po3-ea.mq5`, notes §38) | Time + Price | The ±2 window this section asks for, on a **different count** from the be15 filter above. Candles from a **calendar anchor** rather than bars since a Kijun touch, judged within ±2 of the *nearest* kihon number — nearest and either side, because a turn due at 9 is not cancelled by the candle after it. The gate is fixed to the **daily H1 setup**: H1 counted from the **day** open, judged against candles **9 and 17** — the only two numbers a day's ~23–24 candles can reach, and hardcoded as a pair precisely because 26 is *not* reachable while a nearest-number test would still fire on it at the day's last candle. This began as a ladder of timeframes judged N-of-M; the rung choice has been made, so the ladder and its machinery were removed and the gate is now one fixed rule (the multi-rung version is preserved in commit `c220059`). It gates entries ahead of the alignment stack. A PO3 level gate behind it supplies the price half: room to the next 3ⁿ level measured in the tier's own ATR, and the level as the take profit. |
