//+------------------------------------------------------------------+
//| EXPERIMENT — M5-BASE, KEEP TIERS (notes §55, magic 20260880)      |
//| A fork of the M5-base stack (§54) with one change: a bigger tier  |
//| no longer closes the smaller ones. With InpKeepLowerTiers on      |
//| (default), every aligned flat tier opens on the same bar and a    |
//| running lower-tier trade keeps going to its own exit (cloud       |
//| touch, BE, trail, disaster stop) when a higher tier opens. Up to  |
//| four positions per symbol (M15, M30, H1, H4), one per tier; each  |
//| sized by its own risk %, so tier-1 exposure can reach 36% of      |
//| equity at once. Off = the §54 behaviour (largest tier only, lower |
//| tiers closed on supersede).                                       |
//|                                                                  |
...
input bool   InpKeepLowerTiers   = true;   // KEEP TIERS: every aligned tier opens; a bigger tier no longer closes the smaller ones
...
int MAGIC = 20260880;   // M5-base keep-tiers experiment (M5-base parent runs 20260879)
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      if(!symBlockedUnknown[s] && SpreadOK(syms[s]))
      {
         for(int l = LEVELS - 1; l >= 0; l--)
         {
            ...
            // KEEP TIERS: open this tier now and carry on down the stack;
            // nothing running is closed.
            if(InpKeepLowerTiers)
            {
               double kLots = RiskLots(s, l);
               CapLotsToMargin(syms[s], (st == 1), kLots);
               if(!OpenLevel(s, l, st, kLots, via))
