//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA — LEAN 3-TIER STACK                   |
//| EXPERIMENTAL BUILD — not deployed. Magic 20260868.                |
//|                                                                  |
//| FORK OF ichimoku-h4-m1-vps-ea.mq5 (the live VPS build, magic      |
//| 20260858), which is left untouched. The change is the TIER SET:   |
//| the parent traded FIVE tiers — M5, M15, M30, H1, H4 — and this    |
//| build trades THREE, chosen for separation rather than coverage:   |
//|                                                                  |
//|        M5, M15, H1        (ratios 3x then 4x)                     |
//|                                                                  |
//| WHY THESE THREE. The alignment chain is a strict AND, so every    |
//| tier added to it multiplies the rarity of every chain above it    |
//| while buying less and less independent information — adjacent     |
//| timeframes are highly correlated, and a rung earns its place by   |
//| what it adds, not by existing. M30 and H4 were the weakest links  |
//| by that measure: M30 sits between M15 and H1 (2x spacing) and H4  |
//| is ALREADY a gate on every trade through the H4 bias, which runs  |
//| the identical CheckAlign test the H4 rung would have run. Their   |
//| removal shortens every chain and gives the surviving tiers back   |
//| their fire rate.                                                  |
//|                                                                  |
//| H4 IS STILL READ — AS A BIAS ONLY. "H4 is not a tier" does not    |
//| mean "H4 is not used": tfs[] still carries H4 at index 4 and the  |
//| H4 bias filter still requires it aligned with the trade before    |
//| any entry (with the H1 stand-in for M5 and M15 while H4 is flat). |
//| What is gone is H4 as a tradable LEVEL — no risk row, no ATR      |
//| handle, no exit, no position. The same is true of M30: dropped    |
//| from the tier set, and nothing else reads it.                     |
//|                                                                  |
//| THE CHAINS, bottom-up from M1:                                    |
//|          Tier M5 : M1 + M5 aligned                    -> open      |
//|          Tier M15: M1 + M5 + M15 aligned              -> open      |
//|          Tier H1 : M1 + M5 + M15 + H1 aligned         -> open      |
//|          M1 alone never trades — it is the first leg.             |
//|          H4 gates DIRECTION on all three (or the H1 stand-in for  |
//|          M5 and M15 while H4 is flat).                            |
//|                                                                  |
//| WHAT THIS CHANGES BESIDES THE TIER COUNT — two inherited rules    |
//| pointed at the old top tier and now point at H1:                  |
//|   * the D1 FILTER. In the parent it gated the H4 tier, the top    |
//|     one; the code reads "the top tier" as LEVELS - 1, so it now   |
//|     gates H1. The parent's H1 tier had NO D1 filter, so this is   |
//|     a real behaviour change for H1: its entries additionally      |
//|     need D1 aligned. Set InpD1Filter=false for the parent's H1    |
//|     behaviour.                                                    |
//|   * the TIGHT BREAK-EVEN AND FULL CHANDELIER bucket (lvl >= 3 in  |
//|     the parent, meaning H1 and H4) is now lvl >= LEV_H1, so H1    |
//|     keeps exactly the treatment it had. M5 and M15 keep the       |
//|     looser spike-gated trail they had. Nothing moves.             |
//|                                                                  |
//| RISK. Three tier-1 rows instead of five: M5 1%, M15 1%, H1 10% —  |
//| 12% of equity if all three ran at once, against the parent's 37%  |
//| for five tiers. Per §24 that aggregate is reachable, because a    |
//| higher tier already running is never closed by a smaller signal,  |
//| and the R3 disaster stop sits at 8x ATR while sizing assumes 2x — |
//| a 4x multiplier on the worst case.                                |
//|                                                                  |
//| Magic: 20260868 — carried from the earlier tier-set drafts of     |
//| this experiment (never deployed); distinct from the live build    |
//| (20260858), the desktop twin (20260860) and every other fork.     |
//| CAUTION: if an earlier draft of this experiment ever ran on this  |
//| account, close its positions first. Comments this build manages   |
//| are "Exp Buy/Sell M5", "M15" and "H1"; a leftover "Exp Buy M30",  |
//| "M10", "M20", "H2", "H4" or "M2" would be flagged by R2 as        |
//| unmanageable and block that symbol until it is gone.              |
//| Author: Neo Malesa                                                |
...
//| top-down agreement.                                               |
//| Entry (FORK — three tiers): per-TF alignment (price + chikou      |
//|        above/below tenkan, kijun and cloud), checked BOTTOM-UP: a |
//|        tier opens only when the full stack M1..tier TF is aligned |
//|        in one direction:                                          |
//|          Tier M5 : M1 + M5 aligned              -> open trade     |
//|          Tier M15: M1 + M5 + M15 aligned        -> open trade     |
//|          Tier H1 : M1 + M5 + M15 + H1 aligned   -> open trade     |
//|        M1 alone never trades — it is only the start of the stack. |
//|        H4 is NOT a tier but is still read: the H4 bias requires   |
//|        it aligned with the trade before any entry, with the H1    |
//|        stand-in for M5/M15 while H4 is flat.                      |
//|        M1 alone never trades — it is only the start of the stack. |
...
//| Risk:  one position per LEVEL per symbol, but consolidation:      |
//|        when several tiers align at once only the LARGEST opens,   |
//|        and any smaller tier already running on the symbol is      |
//|        closed first. That is NOT one position per symbol: a       |
//|        higher tier already running is never closed by a smaller   |
//|        signal, so up to three tiers can run at once (the parent's |
//|        header claimed otherwise; EXPERIMENTAL-NOTES §24 corrects  |
//|        it). Every trade risks a                                   |
//|        fixed % of the ACTUAL equity at entry, de-risking as the   |
//|        account grows: tier 1 below $7000 (M5/M15 1%, H1 10% —     |
//|        12% if all three ran together), tier 2 half regime         |
//|        $7000-$13000 (0.5/0.5/5), tier 3 tiny regime $13000+       |
//|        (0.1/0.1/1), against the reference distance                 |
...
//| Magic (FORK): 20260868 — fresh relative to the live build and    |
//|        the desktop twin, so nothing here adopts or manages their  |
//|        positions. The parent's own magic note is kept below.      |
//|                                                                  |
...
//| Magic (parent): 20260858 — the live build's own number, kept here |
//|        so the fork's diff stays legible. THIS FORK RUNS 20260868. |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
input double InpRiskPctH1       = 10.0;   // H1   — tier 1 (the top tier of this stack)
...
// Highest tier permitted to enter on the H1 stand-in bias. The H1 tier itself
// is deliberately absent: the stand-in IS H1's direction, so letting the H1 tier
// fall back on it would be validating H1 against itself. H1 therefore needs H4 —
// and, through the D1 filter, the daily — on its own.
enum ENUM_H1_BIAS_TIER { H1TIER_M5 = 0, H1TIER_M15 = 1, H1TIER_H1 = 2 };
...
input ENUM_H1_BIAS_TIER InpH1BiasMaxTier = H1TIER_M15;     // Highest tier allowed on the H1 bias (0=M5, 1=M15, 2=H1)
...
#define MAX_SYMS 60
#define LEVELS   3      // tradable levels: M5, M15, H1 — the lean stack
#define TFS      5      // everything READ: M1, M5, M15, H1, H4
#define IDX_H1   3      // index of H1 in tfs[] — the top tier AND the stand-in bias TF
#define IDX_H4   4      // index of H4 in tfs[] — the primary bias TF, NOT a tier

// Level indices. A level's timeframe is tfs[lvl + 1], so M5 = 0 -> tfs[1],
// M15 = 1 -> tfs[2], H1 = 2 -> tfs[3]. H4 sits at tfs[4] and is read only by the
// bias filter: it has no level, no risk row and no ATR handle. Named constants
// are used wherever a level is compared by number, so the tier count can change
// without a bare literal quietly changing meaning.
#define LEV_M5   0
#define LEV_M15  1
#define LEV_H1   2

ENUM_TIMEFRAMES tfs[TFS] = { PERIOD_M1, PERIOD_M5, PERIOD_M15, PERIOD_H1, PERIOD_H4 };
string          tfName[TFS] = { "M1", "M5", "M15", "H1", "H4" };
...
int MAGIC = 20260868;   // FORK: lean M5/M15/H1 stack — fresh; parent VPS 20260858, desktop twin 20260860
...
int OnInit()
{
   ...
   // The lean tier set is the whole point of this fork, and it is not what the
   // parent's journal lines look like, so the ladder is stated once at start —
   // including the timeframes that are READ but do NOT trade.
   string tierList = "";
   for(int l = 0; l < LEVELS; l++)
      tierList += (l > 0 ? ", " : "") + tfName[l + 1];
   Print("Tier stack (" + IntegerToString(LEVELS) + " tradable tiers): " + tierList +
         " — a tier opens only when every timeframe from M1 up to it is aligned. "
         "H4 is READ but is NOT a tier: it gates direction through the H4 bias only.");
...
//==============================================================
// Daily Bias Filter (TOP tier): D1 is the bias for the top tier's
// trades. In the parent that was the H4 tier; with the lean stack
// the top tier is H1, and the caller reads "the top tier" as
// LEVELS - 1, so this now gates H1. Same alignment semantics as
// CheckAlign but on D1 — D1 bullish (price + chikou above tenkan,
// kijun and cloud) allows only top-tier buys, D1 bearish only
// sells. A D1 close INSIDE the cloud (or unreadable) returns 0 —
// no new top-tier trades then. NOTE: the parent's H1 tier had no
// D1 filter, so this IS a behaviour change for H1; InpD1Filter =
// false restores the parent's H1 exactly.
//==============================================================
...
bool LevelCloudBiasOK(int s, int lvl, int dir)
{
   ...
   if(lvl == LEV_M5) return CloudBiasOK(s, 0, dir);     // M1 (below the M5 tier) — current AND future
...
bool H1BiasTier(int lvl)
{
   ...
   if(lvl >= LEV_H1)                   return false;   // never the H1 tier — the stand-in IS H1's direction
...
// H1 10%), half regime between the tiers (0.5/0.5/5), and the
// tiny regime at InpRiskTier3At and above (0.1/0.1/1). The tier
// rows are the parent's own for M5, M15 and H1 — the lean stack
// does not invent risk, it removes the M30 and H4 rows along
// with those tiers, so the tier-1 total falls from 37% to 12%.
// Sizing measures the % against a reference distance of
// ATR(level TF) x InpRiskATRMult, the same ATR-based sizing
// philosophy as the H4 VPS build. More equity -> more risk money
// -> bigger lots at the same ATR distance; no multipliers on top.
...
double LevelRiskPct(int lvl)
{
   ...
   switch(lvl)
   {
      case LEV_M5:  return t3 ? InpRiskPctM5_T3  : t2 ? InpRiskPctM5_T2  : InpRiskPctM5;
      case LEV_M15: return t3 ? InpRiskPctM15_T3 : t2 ? InpRiskPctM15_T2 : InpRiskPctM15;
      case LEV_H1:  return t3 ? InpRiskPctH1_T3  : t2 ? InpRiskPctH1_T2  : InpRiskPctH1;
   }
   return 0.0;
}
...
void ManageLevelProtection(int s, int lvl)
{
   ...
   double beATR = (lvl >= LEV_H1) ? InpBEProfitH1H4 : InpBEProfitATR;
   if(!beMoved[s][lvl])
   {
   ...
   double armATR = (lvl >= LEV_H1) ? InpTrailActivateATR : InpSpikeLockATR;
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      if(!symBlockedUnknown[s] && SpreadOK(syms[s]))
      {
         for(int l = LEVELS - 1; l >= 0; l--)
         {
            ...
            // TOP tier: D1 must carry the same bias (D1 in the cloud = no entry).
            // This was the H4 tier in the parent; the top tier is now H1 and the
            // test follows LEVELS - 1, so H1 gains a D1 filter the parent's H1
            // tier did not have. InpD1Filter=false restores the parent's H1.
