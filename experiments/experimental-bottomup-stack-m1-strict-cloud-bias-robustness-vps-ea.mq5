...
//|                                                                  |
//| ROBUSTNESS FORK (experimental, 2026-08-21) — a verbatim copy of  |
//| the live VPS build above plus five hardening changes (review     |
//| recommendations R2-R6). NOT deployed; manages only its own magic.|
...
//|   R6 PORTING NOTES for promoting into BOTH production builds     |
//|      (VPS 20260858 / desktop 20260860): the only CODE changes vs |
//|      the parent are in the input groups, the globals around      |
//|      MAGIC, LogUnknownOnce(), SyncStateFromPositions(),          |
//|      CapLotsToMargin(), OpenLevel(), ManageLevelProtection() and |
//|      the single entry-gate line in OnTick(); this banner is the  |
//|      only other difference. Diff against the parent before any   |
//|      promotion and re-check the AGENTS.md twin rule afterwards.  |
//|      Deliberately NOT included (user decision): recommendation   |
//|      R1, the supersede invariant guard that would abort a new    |
//|      entry while an existing higher-tier position survives its   |
//|      close attempt.                                              |
//| Magic: 20260863 — FRESH, so this experiment can never manage     |
//|        positions of the live builds (20260858 / 20260860), the   |
//|        retired builds, or any other experiment.                  |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
int MAGIC = 20260863;   // robustness fork of the VPS build — FRESH number, shares positions with nothing
