//+------------------------------------------------------------------+
//| EXPERIMENT — M5-BASE, RISK x1.25 (notes §56, magic 20260881)      |
...
//| 0.125/0.125/0.25/1.25/2.5. The regime thresholds, the 2 x ATR     |
//| sizing distance and the 80% margin cap are unchanged. The risk    |
//| figures quoted further down are the parent's (x1.0).             |
//|                                                                  |
//| PARENT — M5-BASE STACK (notes §54)                                |
//| A fork of the live VPS build as it stands on 2026-09-23 (M1-     |
//| strict cloud bias, robustness pack, M5 tier off). The one change: |
//| the stack starts at M5, not M1. M1 takes no part in alignment:    |
//|          Tier M15: M5 + M15 aligned             -> open trade     |
//|          Tier M30: M5 + M15 + M30 aligned       -> open trade     |
//|          Tier H1 : M5 ... H1 aligned            -> open trade     |
//|          Tier H4 : M5 ... H4 aligned            -> open trade     |
//| M5 alone never trades — it is only the start of the stack, as M1  |
//| was. The cloud gate is the live one: tier TF and the TF below it, |
//| future cloud only. InpBaseCloudFull (off by default) moves the    |
//| old M1 rule onto the new base: the M15 tier then needs M5 twisted |
//| its way at both the current bar and the far end of the future     |
//| cloud. Bias, risk, exits and protection are the live build's.     |
//| M1 bars still pace the loop (once per closed M1 bar), so exits    |
//| are checked exactly as often as live. The description below is   |
//| the parent's and still speaks of M1 in the stack.                 |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
...
input double InpRiskPctM5       = 1.25;   // M5   — tier 1 (equity < Tier2At)
input double InpRiskPctM15      = 1.25;   // M15  — tier 1
input double InpRiskPctM30      = 6.25;   // M30  — tier 1
input double InpRiskPctH1       = 12.5;   // H1   — tier 1
input double InpRiskPctH4       = 25.0;   // H4   — tier 1
input double InpRiskPctM5_T2    = 0.625;  // M5   — tier 2 (half regime)
input double InpRiskPctM15_T2   = 0.625;  // M15  — tier 2
input double InpRiskPctM30_T2   = 3.125;  // M30  — tier 2
input double InpRiskPctH1_T2    = 6.25;   // H1   — tier 2
input double InpRiskPctH4_T2    = 12.5;   // H4   — tier 2
input double InpRiskPctM5_T3    = 0.125;  // M5   — tier 3 (equity >= Tier3At)
input double InpRiskPctM15_T3   = 0.125;  // M15  — tier 3
input double InpRiskPctM30_T3   = 0.25;   // M30  — tier 3
input double InpRiskPctH1_T3    = 1.25;   // H1   — tier 3
input double InpRiskPctH4_T3    = 2.5;    // H4   — tier 3
...
input bool   InpBaseCloudFull    = false;  // M15 tier: M5 (the base) needs current AND future cloud, like M1 did in the live build
...
#define IDX_BASE 1      // M5-BASE: the stack starts here (M5), not at M1
...
int MAGIC = 20260881;   // M5-base risk x1.25 experiment (M5-base parent runs 20260879)
...
{
   int dir = CheckAlign(s, IDX_BASE);           // M5-BASE: M1 is not in the chain
   if(dir == 0) return 0;

   for(int t = IDX_BASE + 1; t <= topIdx; t++)
   {
...
bool LevelCloudBiasOK(int s, int lvl, int dir)
{
   ...
   // M5-BASE: the M5 tier never trades, so the M1 branch is gone. The base
   // (M5, below the M15 tier) takes the full check only when asked.
   if(lvl == 1 && InpBaseCloudFull) return CloudBiasOK(s, IDX_BASE, dir);
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      if(!symBlockedUnknown[s] && SpreadOK(syms[s]))
      {
         for(int l = LEVELS - 1; l >= 0; l--)
         {
            ...
            // M5-BASE: M5 is the start of the stack and never trades alone.
            if(l == 0) continue;
