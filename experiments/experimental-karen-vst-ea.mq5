//+------------------------------------------------------------------+
//| Experimental: Karen Peloille VST (very short term) EA            |
//| A fork of experimental-karen-multitf-ea.mq5 using her "Short     |
//| term" time-frame table from "Trading with Ichimoku" ch. 4:       |
//|    Analysis = 60 minutes, Strategy = 15 minutes,                 |
//|    Position management = 5 minutes.                              |
//| ("So, I consider these two time frames as being the most         |
//|  relevant for Ichimoku... 240-minute and 15-minute... they both  |
//|  provide the clearest signals via Kijun breakouts.")             |
//| The engine is identical to the multi-TF build: Kijun break on    |
//| the strategy TF with Lagging Span validation, pullback to the    |
//| Tenkan on the management TF, Tenkan-cross exit, ATR stop, trail  |
//| and BE30. Only the time frames, magic number and order comments  |
//| differ, so both builds can run on one account at the same time.  |
//|                                                                  |
...
//    Defaults are her "short term" table: 60 / 15 / 5 minutes.
input group  "Timeframes"
input ENUM_TIMEFRAMES InpAnalysisTF = PERIOD_H1;    // Analysis TF (general trend: prices vs cloud)
input ENUM_TIMEFRAMES InpSignalTF   = PERIOD_M15;   // Strategy TF (Kijun break = the signal)
input ENUM_TIMEFRAMES InpEntryTF    = PERIOD_M5;    // Management TF (pullback entry + Tenkan exit)
...
ENUM_TIMEFRAMES tfs[TF_COUNT] = { PERIOD_H1, PERIOD_M15, PERIOD_M5 };
...
int MAGIC = 20260841;   // fresh — no other EA uses this
...
int OpenPositions(int s, bool isBuy, double dist, double tp, int count, double lots, double &firstFill)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, tp, "Buy Karen VST")
                      : trade.Sell(lots, sym, price, sl, tp, "Sell Karen VST");
      if(!ok) break;
         ...
         if(st != 0)
         {
            ...
            if(filled > 0)
            {
               ...
               string msg = PCTime() + " | " + action + " " + syms[s] +
                            ...
                            " @ " + DoubleToString(lots, 2) + " (Karen VST)";
               if(tp > 0)
