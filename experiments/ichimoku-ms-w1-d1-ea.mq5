//+------------------------------------------------------------------+
//| Ichimoku MS-W1-D1 Alignment EA                                   |
//| Entry: MS→W1→D1 (highest to lowest) price+chikou all above/below  |
//|        tenkan,kijun,cloud                                        |
//| Exit:  InpExitTF close crosses kijun against trade direction,     |
//|        or ATR-based protective stop loss                         |
//| Note:  Very rare, multi-week/month trades. Evaluate once per D1  |
//|        bar close. Intended first for backtesting; the same       |
//|        alignment logic can later be folded into the existing     |
//|        EAs as a function.                                        |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
input ENUM_TIMEFRAMES InpATRTF    = PERIOD_D1;   // ATR timeframe for stop distance
...
input ENUM_TIMEFRAMES   InpExitTF = PERIOD_D1;   // Exit when close crosses kijun on this TF (e.g. D1 or W1)
...
// Highest-to-lowest alignment stack: MS (monthly) → W1 (weekly) → D1 (daily)
ENUM_TIMEFRAMES tfs[TF_COUNT] = {
   PERIOD_MN1, PERIOD_W1, PERIOD_D1
};

int      ich[MAX_SYMS][TF_COUNT];
int      ichExit[MAX_SYMS];  // exit-reference timeframe kijun
int      atr[MAX_SYMS];
string   syms[MAX_SYMS];
int      symsCount = 0;
datetime lastD1bar[MAX_SYMS];
datetime noReentryUntil[MAX_SYMS];
datetime lastEquityBar = 0;
...
int OnInit()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      ichExit[s] = iIchimoku(syms[s], InpExitTF, Tenkan, Kijun, SenkouB);
      if(ichExit[s] == INVALID_HANDLE) return(INIT_FAILED);

      atr[s] = INVALID_HANDLE;
      if(InpUseStopLoss)
      {
         atr[s] = iATR(syms[s], InpATRTF, InpATRPeriod);
...
//==============================================================
// Entry Check: all timeframes (MS→D1) aligned same direction
//==============================================================
...
//==============================================================
// Exit Check: price closed on wrong side of kijun on InpExitTF
//==============================================================

bool CheckExit(int s, int dir)
{
   double kij[1];
   if(CopyBuffer(ichExit[s], 1, 1, 1, kij) <= 0) return false;

   MqlRates rt[];
   if(CopyRates(syms[s], InpExitTF, 1, 1, rt) <= 0) return false;
...
// ATR(InpATRTF) * multiplier, widened to the broker's minimum stop distance
// if needed. Returns false when the ATR value is unavailable so the caller
...
int OpenPositions(int s, bool isBuy, double dist, int count, double lots)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, 0, "Buy MS-W1-D1")
                      : trade.Sell(lots, sym, price, sl, 0, "Sell MS-W1-D1");
...
void OnTick()
{
   // Equity alert: fire only on a new D1 bar (CheckEquityAlert self-guards on day-of-week)
   MqlRates d1[];
   if(symsCount > 0 && CopyRates(syms[0], PERIOD_D1, 0, 1, d1) > 0 && d1[0].time != lastEquityBar)
   {
      lastEquityBar = d1[0].time;
      CheckEquityAlert();
   }
   ...
   for(int s = 0; s < symsCount; s++)
   {
      // Per-symbol D1 bar gating — only act on a new closed D1 bar for this symbol
      MqlRates d1b[];
      if(CopyRates(syms[s], PERIOD_D1, 0, 2, d1b) < 2) continue;
      ArraySetAsSeries(d1b, true);
      if(d1b[1].time == lastD1bar[s]) continue;
      lastD1bar[s] = d1b[1].time;

      // Sync position state once per tick on the first new D1 bar instead of
      // rebuilding it on every single tick.
      if(!synced) { SyncStateFromPositions(); synced = true; }

      // Exit check: close all when close crosses kijun against direction on InpExitTF
      if(state[s] != 0 && CheckExit(s, state[s]))
      {
         string side = (state[s] == 1) ? "Long" : "Short";
         string msg  = PCTime() + " | Close " + syms[s] + " " + side + " (kijun crossed)";
      ...
      // Entry check: all timeframes MS→D1 must align, spread must be sane
         ...
         if(st != 0)
         {
            ...
            if(filled > 0)
            {
               ...
               string msg = PCTime() + " | " + action + " " + syms[s] +
                            ...
                            " @ " + DoubleToString(lots, 2) + " (MS-W1-D1)";
               Print(msg); Alert(msg); SendNotification(msg);
            }
            else
