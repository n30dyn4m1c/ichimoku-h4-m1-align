...
//| XAUUSDc CENT-ACCOUNT FORK (Exness), LIVE RISK REGIME, MIN 0.01    |
//| A copy of the live VPS build (ichimoku-h4-m1-vps-ea.mq5, M5 tier  |
//| off, robustness pack on) re-set for the Exness cent symbol        |
//| XAUUSDc, for strategy-tester work before any live use. Trading    |
//| logic is the live build's, byte for byte. Four changes, all about |
//| the account and the instrument, not the strategy:                 |
//|   1. Symbols preset to "XAUUSDc".                                  |
//|   2. LIVE RISK REGIME ON A CENT ACCOUNT: sizing is the live       |
...
//|      the same numbers would be a tenth of the intended distance — |
//|      a BE stop inside the spread and a spread gate that blocks    |
//|      almost everything. Those inputs are now in 2-DECIMAL GOLD    |
//|      POINTS (0.01 of price) and PtScale() converts them to the    |
//|      symbol's own points (x10 on a 3-decimal feed, x1 on a        |
//|      2-decimal one). OnInit prints the resolved values.           |
//|   4. Fresh magic 20260884, so it never touches the live build's   |
//|      positions if both ever run on one terminal.                  |
//| Parent description follows (magic lines refer to the parent).     |
...
//--- Input Parameters ---
input string Symbols  = "XAUUSDc";
input int    Tenkan   = 9;
input int    Kijun    = 26;
input int    SenkouB  = 52;
input int    Slippage = 30;          // Max slippage, in 2-decimal gold points (0.01 of price) — scaled by PtScale()
...
input int    InpMaxSpreadPoints  = 60;     // Max spread to allow entry, in 2-decimal gold points (0.01 of price; 0 = no limit)
...
input int    InpBECoverPoints     = 15;    // Distance beyond entry for the BE stop, in 2-decimal gold points (0.01 of price)
...
int MAGIC = 20260884;   // XAUUSDc cent fork of the live VPS build (live runs 20260858)
...
// XAUUSDc fork: symbol points per 2-decimal gold point (0.01 of price) —
// 10 on a 3-decimal feed (Exness), 1 on a 2-decimal one (GOLDm#). Every
// point input of the live build is scaled through this, so the same input
// means the same price distance on either feed.
int PtScale(string sym)
{
   double point = SymbolInfoDouble(sym, SYMBOL_POINT);
   if(point <= 0) return 1;
   return (int)MathMax(1, MathRound(0.01 / point));
}
...
int OnInit()
{
   ...
   trade.SetDeviationInPoints(Slippage * PtScale(syms[0]));
   for(int s = 0; s < symsCount; s++)
   {
      int    sc  = PtScale(syms[s]);
      double pt  = SymbolInfoDouble(syms[s], SYMBOL_POINT);
      int    dg  = (int)SymbolInfoInteger(syms[s], SYMBOL_DIGITS);
      Print(syms[s] + ": digits " + IntegerToString(dg) + ", point scale x" + IntegerToString(sc) +
            " | spread cap " + IntegerToString(InpMaxSpreadPoints * sc) + " pts (" +
            DoubleToString(InpMaxSpreadPoints * sc * pt, dg) + ")" +
...
bool SpreadOK(string sym)
{
   ...
   return SymbolInfoInteger(sym, SYMBOL_SPREAD) <= InpMaxSpreadPoints * PtScale(sym);
}
...
void ManageLevelProtection(int s, int lvl)
{
   ...
   if(!beMoved[s][lvl])
   {
      ...
      if(armed)
      {
         double cover = InpBECoverPoints * PtScale(syms[s]) * point;
         double slNew = isLong ? entryPrice[s][lvl] + cover
                               : entryPrice[s][lvl] - cover;
