//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA — MINIMAL (structural fixes only)     |
//|                                                                  |
//| Fork of experimental-bottomup-stack-ea-very-profitable.mq5       |
//| (the "very profitable" snapshot, magic 20260848).                |
//|                                                                  |
//| WHY THIS BUILD EXISTS: backtests of the windows-laptop and the   |
//| overextension-protection variants showed the FILTERED builds     |
//| performed WORSE than the aggressive snapshot — fewer trades,     |
//| lower profit factor. The edge is in trend continuation: entries  |
//| during stretches (no ADX filter, no overextension gate, no news  |
//| blackout, no pullback guard) are where the big runners come      |
//| from. This build keeps the snapshot's EXACT trading logic and    |
//| adds only structural fixes that do not change the trade flow:    |
//|                                                                  |
//| STRUCTURAL FIXES:                                                |
//|  * ONE POSITION PER SYMBOL enforced in BOTH directions — the     |
//|    snapshot only closed smaller tiers when a LARGER tier opened; |
//|    a running H4 with M1..H1 still aligned let the lower tiers    |
//|    pile on top of it (up to 20%+10%+5%+1%+1% of equity stacked,  |
//|    all naked). Entries now require the whole symbol flat.        |
//|  * DISASTER STOP (InpDisasterStopATR, default 4 x ATR) — a wide  |
//|    real SL attached at entry that only ever TIGHTENS via BE/     |
//|    trail. Caps the tail risk of the naked-runner design without  |
//|    cutting any trades.                                           |
//|  * PER-TICK KUMO-TOUCH EXIT — the snapshot only checked the      |
//|    cloud touch once per closed M1 bar (up to 60 s late); now     |
//|    the exit fires immediately on every tick.                     |
//|  * OPTIONAL re-entry cooldown (InpReentryCooldownMin, default 0  |
//|    = OFF, exactly the snapshot) — turn on (e.g. 30) only if the  |
//|    cloud-edge whipsaw shows up in your backtest.                 |
//|  * ALIGNMENT CACHE — CheckAlign runs once per TF per M1 bar      |
//|    instead of up to 21x per symbol (no behavior change).         |
...
//| (off), BE + chandelier, 3-tier risk regimes, entry               |
//| consolidation, spread filter, notifications.                     |
//| NO ADX filter. NO H4 overextension gate. NO news blackout.       |
//| NO pullback guard.                                               |
//| Magic: 20260852                                                  |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
...
input group  "Stop Loss Policy (structural)"
input bool   InpUseInitialSL    = false;   // Attach a real entry SL at ATR x InpRiskATRMult (false = runner config)
input double InpDisasterStopATR = 4.0;     // Wide disaster stop at ATR x this (0 = off) — caps tail risk while runners run

input group  "Re-entry (structural)"
input int    InpReentryCooldownMin = 0;    // Per-level cooldown (min) after an exit before re-entry (0 = off, snapshot behavior)
...
int MAGIC = 20260852;   // fresh — minimal build (20260848 = very-profitable snapshot, 20260849 = office-pc, 20260850 = windows-laptop, 20260851 = overextension-protection)
...
void OnTick()
{
   ...
   // levels with an open position pay the (2 CopyBuffer) cost. Failed
   // closes are retried at most once every 5 seconds (no spam).
   for(int s = 0; s < symsCount; s++)
   {
      for(int l = 0; l < LEVELS; l++)
      {
   ...
   // Everything below runs once per closed M1 bar (entries/protection
   // don't need tick resolution).
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      // Entry scan — EXACTLY the snapshot's logic (no ADX, no overextension
      // gate, no news, no pullback guard) plus the structural fixes:
      // the whole symbol must be flat (a running H4 can no longer have
      // H1/M30/M15/M5 pile on top of it) and the level must be out of its
      // post-exit cooldown. When several tiers align at once, the LARGEST
      // one opens.
      int topTier = -1;
      int topDir  = 0;
      if(SpreadOK(syms[s]) && SymbolFullyFlat(s))
      {
