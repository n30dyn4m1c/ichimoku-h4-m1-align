...
//|     left (strictly beyond) and InpLiqRight on the right (at least |
//|     as far), searched over the last InpLiqLookback candles; it is |
...
//==============================================================
// Liquidity Target (EXPERIMENT): unraided swing highs/lows, the
// po3-levels indicator's rules (§52) ported to the EA. A swing
// high stands strictly above the InpLiqLeft candles before it and
// at least as high as the InpLiqRight candles after it (so of a
// run of equal highs the oldest is the swing); lows mirror it. A
// swing is confirmed only once its right-hand candles have closed.
// It is unraided while no later wick has traded BEYOND it — the
...
// candle, the indicator's LiqScan. Returns 0.0 when there is none.
...
bool OpenLevel(int s, int lvl, int dir, double lots, string via, double tp, string tgt)
{
   ...
   // Liquidity target: re-check the broker distance against the side that
   // will trip it (a long's TP on the BID, a short's on the ASK).
   if(tp > 0.0)
   {
      double point   = SymbolInfoDouble(sym, SYMBOL_POINT);
      double minDist = SymbolInfoInteger(sym, SYMBOL_TRADE_STOPS_LEVEL) * point;
      bool   tpValid = (dir == 1) ? (tp > SymbolInfoDouble(sym, SYMBOL_BID) + minDist)
                                  : (tp < SymbolInfoDouble(sym, SYMBOL_ASK) - minDist);
      if(!tpValid) { tp = 0.0; tgt = "none"; }
   }
   ...
   if(ok)
   {
      ...
      string msg = PCTime() + " | " + action + " " + sym + " " + tfName[lvl + 1] +
                   " @ " + DoubleToString(lots, 2) + " (bottom-up, bias " + via + ", target " + tgt + ")";
...
void ManageLevelProtection(int s, int lvl)
{
   ...
   double tpCur = PositionGetDouble(POSITION_TP);   // liquidity target — carried through every modify
