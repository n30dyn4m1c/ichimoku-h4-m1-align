//+------------------------------------------------------------------+
//| EXPERIMENT — SCALP CAPTURE (magic 20260874, notes §47)           |
//| Forked from the live VPS build (20260858), which is untouched.   |
//| Entries, bias gates, risk and every existing exit are the live   |
//| build's byte for byte. Two things are added:                     |
//|                                                                  |
//|  1. SCALP CAPTURE on the small tiers (M5..InpScalpMaxTier):      |
//|     the live build gives small moves back. BE arms at +1 x ATR   |
//|     but only moves the stop to entry + 15 points; the lower-tier |
//|     trail waits for +2 x ATR. A move that runs +1.9 ATR and      |
//|     turns is closed at break-even, and one that runs +0.9 ATR    |
//|     and turns rides all the way back to the kumo edge. Here, at  |
//|     +InpScalpTP1ATR x ATR(entry) the EA banks InpScalpClosePct % |
//|     of the position, locks the rest at entry + InpScalpLockATR x |
//|     ATR, and arms the chandelier at once (InpScalpTrailATR       |
//|     behind the peak). The runner keeps the kumo-touch exit, so  |
//|     the trend tail the live build lives on is still there.       |
//|     ClosePct = 100 turns it into a pure fixed-target scalp.      |
//|     InpScalpCapture = false restores the live exits exactly.     |
//|                                                                  |
//|  2. PER-TIER REPORT (OnDeinit, Strategy Tester journal): for     |
//|     each tier — trades, win rate, net, profit factor, and the    |
//|     max favourable excursion (MFE, in ATR at entry) bucketed,    |
//|     plus GIVE-BACKS: trades that reached +1 ATR in profit and    |
//|     still closed at or below zero. Run it once with capture off  |
//|     to measure how many scalps the live logic leaves behind.     |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
...
int MAGIC = 20260874;   // scalp-capture experiment — never manages the live build's (20260858) positions
