//+------------------------------------------------------------------+
//| EXPERIMENTAL — CHIKOU EXIT (notes §62)                            |
//| A fork of the live VPS build (ichimoku-h4-m1-vps-ea.mq5, as of    |
//| 2026-09-25) with ONE addition: a second exit condition per tier. |
//| Besides the kumo touch, a trade also closes when the tier TF's   |
//| CHIKOU closes back inside price — the last closed tier-TF close  |
//| is no longer clear of the candle Kijun bars back (long: close <= |
//| that candle's high; short: close >= its low). Entry required     |
//| chikou beyond that candle, so this exits once the chikou half of |
//| the entry test fails. H1 trade -> H1 chikou, M30 -> M30, etc.    |
//| InpChikouExit=false reproduces the live build exactly.           |
//| Magic: 20260885 (own number; the live build keeps 20260858).     |
//|------------------------------------------------------------------|
...
input group  "Chikou Exit (experimental)"
input bool   InpChikouExit    = true;   // Also close when the tier TF's chikou closes back inside price (candle Kijun bars back)
...
int MAGIC = 20260885;   // chikou-exit experiment (live VPS build runs 20260858, desktop 20260860)
...
//==============================================================
// Chikou Exit (experimental): the tier TF's chikou — the last
// closed bar's close, plotted Kijun bars back — has closed back
// inside price. Entry required it clear of that candle (long:
// above its high; short: below its low); once it is not, the
// trade closes. A chikou that drops straight through the candle
// counts too. Uses closed tier-TF bars only, so the answer changes
// only when a tier-TF bar closes; a trade opens only while chikou
// is clear, so the exit cannot fire on the entry's own bar.
//==============================================================

bool ChikouInPrice(int s, int tfIdx, int dir)
{
   int sh      = 1;              // last closed bar
   int chShift = sh + Kijun;     // chikou's chart position for bar sh

   MqlRates rt[];
   if(CopyRates(syms[s], tfs[tfIdx], 0, chShift + 1, rt) <= chShift) return false;
   ArraySetAsSeries(rt, true);
...
void OnTick()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      for(int l = 0; l < LEVELS; l++)
      {
         ...
         // Chikou exit: the tier TF's chikou closed back inside price
         if(InpChikouExit && state[s][l] != 0 && ChikouInPrice(s, l + 1, state[s][l]))
            ExitLevel(s, l, "chikou in price");
