//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA — M1+M2 SCALP TIER, M30 BIAS,         |
//|                               HARD M2/M5 TAKE PROFIT              |
...
//|     trade behind the EA's back. HigherLiveLevelBusy() closes that |
//|     hole for the scalp tier only (InpScalpNeedsFlatSymbol, default |
//|     on). The parent's equivalent case among the five live tiers   |
//|     is deliberately NOT changed here, to keep their behaviour     |
//|     identical — so "at most one position per symbol" holds for    |
//|     the scalp tier, but remains as loose for the live tiers as it |
//|     is in the parent.                                             |
...
input double InpRiskPctM2       = 0.5;    // M2   — tier 1 (equity < Tier2At) — the scalp tier, sized below M5
...
input double InpRiskPctM2_T3    = 0.05;   // M2   — tier 3 (tiny regime)
...
input bool   InpM30ScalpBias = true;       // Require M30 alignment for the scalp tier (its ONLY bias — H4/H1 are not consulted). Off = M1+M2 chain alone
input bool   InpScalpNeedsFlatSymbol = true; // Never scalp while ANY live tier holds a position (keeps the one-position-per-symbol rule; see the note above it)
...
input ENUM_H1_BIAS_TIER InpH1BiasMaxTier = H1TIER_M30;     // Highest tier allowed to enter on the H1 bias (1=M5, 2=M15, 3=M30, 4=H1)
...
input double InpTPPipsM2      = 30.0;    // M2 scalp tier take profit, pips — 30 = 4000.00 -> 4003.00 (0 = no TP on this tier)
input double InpTPPipsM5      = 60.0;    // M5 tier take profit, pips — 60 = 4000.00 -> 4006.00 (0 = no TP on this tier)
...
#define MAX_SYMS 60
#define LEVELS   6      // tradable levels: M2 (scalp), M5, M15, M30, H1, H4
...
#define LVL_M2   0      // the NEW scalp tier — chain M1 + M2, bias M30
#define LVL_M5   1      // first of the five live tiers (was 0)
#define LVL_M15  2      // (was 1)
#define LVL_M30  3      // (was 2)
#define LVL_H1   4      // (was 3) — the BE/chandelier bucket boundary
#define LVL_H4   5      // (was 4) — the top tier, still LEVELS - 1
...
int MAGIC = 20260871;   // M1+M2 scalp tier w/ M30 bias + hard M2/M5 TP — fork of 20260870
...
int OnInit()
{
   ...
   if(InpHardTPEnabled)
   {
      ...
      if(InpTPPipsM2 < 0.0 || InpTPPipsM5 < 0.0)
      {
         Print("Hard TP: InpTPPipsM2 and InpTPPipsM5 cannot be negative. Aborting.");
         return(INIT_FAILED);
      }
   }
   ...
   if(InpRiskTier2At >= InpRiskTier3At)
      Print(PCTime() + " | !! RISK TIER EDGES ARE NOT ASCENDING (InpRiskTier2At < InpRiskTier3At is "
            "required). The tier-2 band is unreachable — fix the inputs.");
   ...
   PrintFormat("   tier  equity range            M2      M5      M15     M30     H1      H4");
   PrintFormat("   1     below %-9.0f         %.3f   %.2f    %.2f    %.2f    %.2f    %.2f",
               InpRiskTier2At, InpRiskPctM2, InpRiskPctM5, InpRiskPctM15, InpRiskPctM30, InpRiskPctH1, InpRiskPctH4);
   PrintFormat("   2     %-6.0f .. %-9.0f    %.3f   %.2f    %.2f    %.2f    %.2f    %.2f",
               InpRiskTier2At, InpRiskTier3At, InpRiskPctM2_T2, InpRiskPctM5_T2, InpRiskPctM15_T2, InpRiskPctM30_T2, InpRiskPctH1_T2, InpRiskPctH4_T2);
   PrintFormat("   3     %-6.0f and above     %.3f   %.2f    %.2f    %.2f    %.2f    %.2f",
               InpRiskTier3At, InpRiskPctM2_T3, InpRiskPctM5_T3, InpRiskPctM15_T3, InpRiskPctM30_T3, InpRiskPctH1_T3, InpRiskPctH4_T3);
   PrintFormat("   ACTIVE at equity %.2f -> M2 %.2f%% (~%.2f%% at stop), M5 %.2f%% (~%.2f%%), H4 %.2f%% (~%.2f%%)",
   ...
   if(InpHardTPEnabled)
   {
      for(int s = 0; s < symsCount; s++)
      {
         ...
         PrintFormat("%s | Hard TP: %s (%d digits) — 1 pip = %.0f points = %s price | "
                     "M2 %.0f pips = %.0f points (%s) | M5 %.0f pips = %.0f points (%s) | M15+ none%s",
      ...
      Print(PCTime() + " | Hard TP: disabled — M2/M5 run on managed exits only, as in the parent");
...
bool LevelCloudBiasOK(int s, int lvl, int dir)
{
   ...
   if(!CloudBiasFarOK(s, lvl + 1, dir)) return false;      // tier TF — M5+ — future cloud only
   if(lvl == LVL_M5) return CloudBiasOK(s, IDX_M1, dir);   // M5 sits on M1 — current AND future
   return CloudBiasFarOK(s, lvl, dir);                     // M15+ — TF below, future cloud only
}
...
//==============================================================
// Does any of the five LIVE tiers hold an open position right now?
//
// Why the scalp tier needs this (and the live tiers do not use it):
// the entry scan SKIPS a level that already holds a position instead
// of evaluating it, and the supersede loop only walks BELOW the tier
// that is opening (`l < topTier`). So a tier that is already running
// is never closed by a new entry, and never blocks one. For the five
// live tiers that is the parent's behaviour and is left exactly as it
// is. For the M2 scalp tier it is a NEW hazard, because M30 — not H4 —
// gives the scalp its direction: with an H4 long running, M1+M2 can
// align SHORT on a later bar, and the scalp would open against a live
// position. On a netting account MT5 would then net the two orders
// together, shrinking or closing the live trade behind the EA's back
// while state[] still reports it open.
//
// So the scalp tier is held to the one-position-per-symbol rule the
// consolidation design already implies. InpScalpNeedsFlatSymbol = false
// restores the parent's looser semantics for the scalp tier alone.
//==============================================================

bool HigherLiveLevelBusy(int s)
{
   for(int k = LVL_M5; k <= LEVELS - 1; k++)     // every live tier; M2 (level 0) deliberately excluded
...
double LevelRiskPct(int lvl)
{
   ...
   switch(lvl)
   {
      ...
      case LVL_M5:  return t3 ? InpRiskPctM5_T3  : t2 ? InpRiskPctM5_T2  : InpRiskPctM5;
      case LVL_M15: return t3 ? InpRiskPctM15_T3 : t2 ? InpRiskPctM15_T2 : InpRiskPctM15;
      case LVL_M30: return t3 ? InpRiskPctM30_T3 : t2 ? InpRiskPctM30_T2 : InpRiskPctM30;
      case LVL_H1:  return t3 ? InpRiskPctH1_T3  : t2 ? InpRiskPctH1_T2  : InpRiskPctH1;
      case LVL_H4:  return t3 ? InpRiskPctH4_T3  : t2 ? InpRiskPctH4_T2  : InpRiskPctH4;
   }
   return 0.0;
}
   ...
   if(tpCur == 0.0 && LevelTPPips(lvl) > 0.0)
   {
      ...
      if(tpHeal > 0.0)
      {
         if(trade.PositionModify(ticket, slCur, tpHeal))
            ...
            Print(PCTime() + " | " + syms[s] + " " + tfName[lvl + 1] + " TP attach failed, retcode " +
                  IntegerToString(trade.ResultRetcode()));
      }
   }
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      if(!symBlockedUnknown[s] && SpreadOK(syms[s]))
      {
         for(int l = LEVELS - 1; l >= 0; l--)
         {
            ...
            if(l == LVL_M2)   // scalp-tier gates: switched on, feed usable, and no live tier running
            {
               if(!InpM2ScalpTier || !scalpReady[s]) continue;
               if(InpScalpNeedsFlatSymbol && HigherLiveLevelBusy(s)) continue;
            }
