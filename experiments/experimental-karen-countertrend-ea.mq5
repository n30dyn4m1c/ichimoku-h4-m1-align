//+------------------------------------------------------------------+
//| Experimental: Karen Peloille Counter-Trend EA                    |
//| Based on "Trading with Ichimoku" ch. 4 (AUDNZD, NZDUSD trades)   |
//| and her article "Trader avec Ichimoku: 3 techniques" (Technique  |
//| A: entry at the SSB of the H4 cloud).                            |
//|                                                                  |
//| The market is in a strong trend on the higher time frames; the   |
//| correction that follows is faded back toward the Kijun of the    |
//| strategy TF. Her words: "It then seems necessary to consider     |
//| shorter time frames" — a downward move is "initiated upon a      |
//| break below the SSB currently below prices. The currency pair    |
//| would in this case fall as far as the 240-minute Kijun." And on  |
//| the entry level: "entrer une position vendeuse sur le niveau de  |
//| cette SSB", with the stop "place au-dessus de la SSB".           |
//|                                                                  |
//| Screens (defaults D1 -> H4 -> H1):                               |
//|   - TREND (D1): prices beyond the D1 cloud, LS validates.        |
//|   - LEVEL (H4): prices beyond the H4 cloud (trend confirmed);    |
//|     the correction has pushed price back to the H4 cloud edge    |
//|     (the SSB side), where the trade is taken.                    |
//|   - BOUNCE (H1): a wick touched the H4 cloud edge within the     |
//|     last InpPullbackBars bars and the last H1 bar closed back    |
//|     on the trend side of the H1 Tenkan.                          |
//| Stop: beyond the H4 cloud edge (the touched level) + buffer,     |
//|        never tighter than the ATR stop.                          |
//| Target: the nearest qualifying level beyond the entry (analysis  |
//|        TF Tenkan/Kijun/cloud, H4 Kijun), with an RR gate of 1.5. |
...
//--- The three screens of her counter-trend system:
//    TF1 TREND = the prevailing trend to trade WITH (cloud side),
//    TF2 LEVEL = trend confirmed + the cloud-edge level to fade,
//    TF3 BOUNCE = entry timing on the rejection candle.
//    Defaults follow her medium-term work: Daily / 240min / 60min.
input group  "Timeframes"
input ENUM_TIMEFRAMES InpAnalysisTF = PERIOD_D1;   // Trend TF (general trend: prices vs cloud)
input ENUM_TIMEFRAMES InpSignalTF   = PERIOD_H4;   // Level TF (cloud-edge level + trend confirmation)
input ENUM_TIMEFRAMES InpEntryTF    = PERIOD_H1;   // Bounce TF (entry timing)

input group  "Karen Counter-Trend Strategy"
input bool   InpSignalCloud       = true;  // Level TF close must be on the trend side of its cloud
input bool   InpSignalChikou      = true;  // Level TF chikou (LS) on the trend side of its Kijun
input bool   InpAnalysisChikou    = true;  // Trend TF chikou (LS) on the trend side of its Kijun
input int    InpPullbackBars      = 8;     // Bounce-TF bars back to find the cloud-edge touch
input bool   InpBounceCandle      = true;  // Bounce bar must close in the trade direction (blue/red body)
input bool   InpChikouEntry       = true;  // Bounce TF chikou (LS) must confirm too
input double InpLevelBufferATR    = 0.5;   // Stop distance beyond the touched level (x ATR)
input double InpMinRR             = 1.5;   // Skip entries with less than this reward:risk to the nearest qualifying level (0 = off)
...
int MAGIC = 20260843;   // fresh — no other EA uses this
...
//==============================================================
// Screen 1 — TREND TF: prices on the trend side of the cloud.
...
//==============================================================
// Screen 2 — LEVEL TF: the trend is confirmed on the strategy TF
// (prices beyond its cloud — "the bearish trend is confirmed with
// prices having crossed the thin cloud") and the Lagging Span
// validates. This TF also supplies the cloud edge that the
// correction must touch and that the stop sits beyond.
//==============================================================

bool LevelScreen(int s, int dir)
{
   if(InpSignalCloud)
   {
      double senA[1], senB[1];
...
//==============================================================
// The level the correction is faded at: the strategy-TF cloud
// edge. In a downtrend (short) that is the cloud TOP — the SSB
// of a bearish cloud, the level of her article's example; in an
// uptrend (long) it is the cloud BOTTOM.
//==============================================================

bool GetCloudEdge(int s, int dir, double &edge)
{
   double senA[1], senB[1];
   if(CopyBuffer(ich[s][IDX_SIGNAL], 2, 1, 1, senA) <= 0) return false;
   if(CopyBuffer(ich[s][IDX_SIGNAL], 3, 1, 1, senB) <= 0) return false;

   edge = (dir ==  1) ? MathMin(senA[0], senB[0])
                      : MathMax(senA[0], senB[0]);
   return edge > 0;
}

//==============================================================
// Screen 3 — BOUNCE TF: the rejection.
// Within the last InpPullbackBars bars a wick touched the H4
// cloud edge ("entrer une position vendeuse sur le niveau de
// cette SSB"), and the last closed bar closed back on the trend
// side of its Tenkan (the correction is over). The touch is read
// against the current cloud edge — an approximation, since the
// edge moves with the cloud (documented in the notes).
//==============================================================

bool BounceScreen(int s, int dir)
{
   double edge;
   if(!GetCloudEdge(s, dir, edge)) return false;
   ...
   // bounce bar = the last closed bar (bar 1), back on the trend side of the Tenkan
   if(dir ==  1)
   {
   ...
   // the correction: a wick reached the cloud edge within the window
   bool touched = false;
   for(int i = 1; i <= win + 1; i++)
   {
      if(dir ==  1 && rt[i].low  <= edge) { touched = true; break; }
      if(dir == -1 && rt[i].high >= edge) { touched = true; break; }
   }
...
//==============================================================
// Full check: TREND + LEVEL + BOUNCE screens on the same side.
// Returns 1 (long), -1 (short) or 0.
//==============================================================

int CheckCounterTrend(int s)
{
   if(AnalysisScreen(s,  1) && LevelScreen(s,  1) && BounceScreen(s,  1)) return  1;
   if(AnalysisScreen(s, -1) && LevelScreen(s, -1) && BounceScreen(s, -1)) return -1;
   return 0;
}
...
// a level the entry screen already crossed (e.g. the H4 Kijun
// when it sits above a fresh counter-trend entry) is on the
// wrong side and is skipped. The nearest qualifying level that
// still leaves at least InpMinRR reward:risk becomes the target,
// front-run by a quarter ATR since price often stalls just short
// of a level ("always leave a few extra points of profit in the
// market"). Returns 0 when no level qualifies — with InpMinRR
// on, the entry is skipped.
//==============================================================
...
// The ATR stop, widened so it always sits beyond the touched cloud
// edge — her rule: "le Stop-Loss est place au-dessus de la SSB".
// Returns false when the ATR value is unavailable so the caller
// skips the entry instead of trading unprotected.
bool GetStopDistance(int s, int dir, double &dist)
{
   dist = 0.0;
   double a[1];
   ...
   double edge;
   if(GetCloudEdge(s, dir, edge))
   {
      double price = (dir == 1) ? SymbolInfoDouble(syms[s], SYMBOL_BID)
                                : SymbolInfoDouble(syms[s], SYMBOL_ASK);
      double levelDist = MathAbs(price - edge) + InpLevelBufferATR * atrVal;
      if(levelDist > dist) dist = levelDist;
   }
...
int OpenPositions(int s, bool isBuy, double dist, double tp, int count, double lots, double &firstFill)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, tp, "Buy Karen CT")
                      : trade.Sell(lots, sym, price, sl, tp, "Sell Karen CT");
      if(!ok) break;
...
void OnTick()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      {
         int st = CheckCounterTrend(s);
         if(st != 0)
         {
            bool   isBuy = (st == 1);
            double dist  = 0.0;
            if(InpUseStopLoss && !GetStopDistance(s, st, dist)) continue;   // ATR unavailable — skip entry
            ...
            if(InpUseTakeProfit)
            {
               ...
               if(InpMinRR > 0 && tp <= 0) continue;   // her RR rule — no qualifying level, no trade
            }
            ...
            if(filled > 0)
            {
               ...
               string msg = PCTime() + " | " + action + " " + syms[s] +
                            ...
                            " @ " + DoubleToString(lots, 2) + " (Karen CT)";
               if(tp > 0)
