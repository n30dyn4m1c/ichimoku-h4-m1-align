//+------------------------------------------------------------------+
//| Ichimoku Structure-Map EA — D1 anchor (experimental)             |
//| Fork of experimental-structure-map-ea.mq5 with the stack shifted  |
//| up one scale: D1 / H4 / H1 / M15 instead of H4 / H1 / M15 / M5.   |
//| The engine is identical — only the defaults and the magic number  |
...
//| Exit:  held on the H1 scale — swing-structure trail, break-even   |
//|        lock, H1 kijun cross, ATR protective stop. The M15 kijun   |
//|        cross of the H4 build stops swing pullbacks out too early. |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
//--- Slot 0 (D1) needs only 56 daily bars of history, under three months,
//--- and the stack contains no exactly-2x pair, so no slot is redundant
//--- under the Ichimoku equivalence (Kijun of TF X = Span B of the 2x
//--- lower TF).
input group  "Structure Map — timeframes and weights"
input ENUM_TIMEFRAMES InpTF1 = PERIOD_D1;      // Slot 1 (highest — daily trend anchor)
input ENUM_TIMEFRAMES InpTF2 = PERIOD_H4;      // Slot 2
input ENUM_TIMEFRAMES InpTF3 = PERIOD_H1;      // Slot 3
...
input int    InpLevelTFs    = 2;  // Bounce off D1 and H4 structures
input int    InpObstacleTFs = 3;  // Measure room down to H1
...
input int    InpReactionBars     = 8;    // Trigger-TF bars back to search for the level touch
input double InpTouchATR         = 0.25; // Level counts as touched within this x ATR(trigger)
input double InpMaxEntryDistATR  = 1.5;  // Max distance from the level at entry (x ATR trigger)
...
input int    InpReentryCooldownSec = 14400; // Min seconds after an exit before re-entering (4h)
...
int OpenLadder(int s, bool isBuy, double sl, double tp, int count, double lots, double &firstFill)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, useSL, useTP, "Buy StructMap D1")
                      : trade.Sell(lots, sym, price, useSL, useTP, "Sell StructMap D1");
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      if(filled > 0)
      {
         ...
         string msg = PCTime() + " | " + action + " " + syms[s] +
                      ...
                      " @ " + DoubleToString(lots, 2) + " (StructMap D1) | " + why;
         Print(msg); SendNotification(msg);
      }
      else
