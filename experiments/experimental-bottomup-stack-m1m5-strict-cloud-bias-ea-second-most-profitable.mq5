//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA (H1 bias) — M1/M5-STRICT CLOUD BIAS   |
//| (EXPERIMENT)                                                      |
//| SECOND MOST PROFITABLE VERSION SO FAR (user report, 2026-08-20) — |
//| the -second-most-profitable suffix marks it as the runner-up to   |
//| the m1-strict most-profitable iteration (m1-strict-cloud-bias-    |
//| ea-most-profitable, $100 -> $14000 on Jan-Aug 2026 data).         |
//| EXPERIMENT — NOT DEPLOYED. Fork of ichimoku-h4-m1-vps-ea.mq5      |
//| (the live VPS build, bottom-up H1-bias stack). Fourth iteration   |
//| of the cloud-bias experiment, forked from the most-profitable     |
//| third iteration (m1-strict-cloud-bias-ea-most-profitable, which   |
//| scaled $100 -> $14000 on Jan-Aug 2026 data per user report).      |
//| Rule: on M1 AND M5 the cloud must be twisted the trade's way at   |
//| BOTH the current bar (last closed bar) and the future cloud (far  |
//| end of the window, Kijun bars ahead) — the full parent check on   |
//| the two smallest timeframes. On M15 and above the CURRENT cloud   |
//| may be any value, bullish or bearish; only the FUTURE cloud must  |
//| be in the trade's direction (unchanged from iteration 3). The     |
//| gate applies to every tier (tier TF + the TF directly below it).  |
//| Lineage: future-cloud-bias -> M1/M5 gate -> M1-strict (most       |
//| profitable) -> this build (M1+M5 strict, M15+ future-only). The   |
//| production VPS file is untouched.                                 |
...
//|        TF and the TF directly below it: M1 and M5 must be twisted  |
//|        the trade's way at both the current bar and the far end of  |
//|        the future cloud; M15 and above need only the far end — the |
...
input bool   InpCloudBiasEnabled = true;   // Require Span A vs Span B bias: M1+M5 current+future must agree; M15+ future cloud only
...
#define IDX_M15  2      // index of M15 in tfs[] — first cloud-free-at-current timeframe
...
int MAGIC = 20260858;   // m1m5-strict-cloud-bias fork (second most profitable), carried over from the superseded cloud-bias forks (live VPS = 20260850, desktop = 20260852)
...
// M1 and M5 timeframes (see CloudBiasTFOK). Unreadable values
// count as blocking.
//==============================================================
...
//==============================================================
// Per-TF cloud rule (EXPERIMENTAL CHANGE): M1 and M5 must be
// twisted the trade's way at BOTH the current bar and the far
// end — the full check (CloudBiasOK). M15 and above need only
// the FUTURE cloud in the trade's direction; their current
// cloud may be any value, bullish or bearish (CloudBiasFarOK).
//==============================================================

bool CloudBiasTFOK(int s, int tfIdx, int dir)
{
   if(tfIdx >= IDX_M15) return CloudBiasFarOK(s, tfIdx, dir);   // M15+ — future cloud only
   return CloudBiasOK(s, tfIdx, dir);                           // M1, M5 — current AND future
}
...
// needs M5 current+future and M15 future; M30/H1/H4 tiers need
