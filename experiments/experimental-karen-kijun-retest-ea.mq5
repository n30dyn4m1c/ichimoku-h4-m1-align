//+------------------------------------------------------------------+
//| Experimental: Karen Peloille Kijun-Retest EA                     |
//| A fork of experimental-karen-multitf-ea.mq5 with the pullback    |
//| target moved from the Tenkan to the Kijun. From "Trading with    |
//| Ichimoku" ch. 4 (EURAUD, "Dynamic Market Reading"): a correction |
//| "goes back to the Kijun, a classic case", and the entry is       |
//| initiated on the candle whose shadow tested the Kijun and was    |
//| validated ("A sell trade is initiated on the first red           |
//| candlestick whose shadow tested the Kijun and validated").       |
//|                                                                  |
//| The engine is identical to the multi-TF build: Analysis (D1)     |
//| trend + Strategy (H4) Kijun break with Lagging Span validation,  |
//| but the entry pullback is deeper — the wick touches the H1       |
//| Kijun (not the Tenkan) and the bounce bar must close back above  |
//| it. Only the pullback screen, magic number and order comments    |
//| differ, so all Karen builds can run on one account at once.      |
//|                                                                  |
...
int MAGIC = 20260842;   // fresh — no other EA uses this
...
//==============================================================
// Screen 3 — ENTRY (management) TF: the Kijun retest.
// Deeper pullback than the Tenkan variant: within the last
// InpPullbackBars bars a wick tested the Kijun ("shadow tested
// the Kijun") while the bar held the Kijun on its close, and
// the last closed bar bounced back on the trade side of the
// Kijun ("and validated"). Optional chikou confirmation.
//==============================================================
...
bool PullbackScreen(int s, int dir)
{
   ...
   // bounce bar = the last closed bar (bar 1), back above the Kijun
   if(dir ==  1)
   {
      if(rt[1].close <= kij[1]) return false;
   ...
   // the pullback: a wick tested the Kijun while the bar held it
   // on the close (the correction stayed on the trend side)
   bool touched = false;
   for(int i = 1; i <= win + 1; i++)
   {
      if(dir ==  1 && rt[i].low  <= kij[i] && rt[i].close > kij[i]) { touched = true; break; }
      if(dir == -1 && rt[i].high >= kij[i] && rt[i].close < kij[i]) { touched = true; break; }
   }
...
int OpenPositions(int s, bool isBuy, double dist, double tp, int count, double lots, double &firstFill)
{
   ...
   for(int i = 0; i < count; i++)
   {
      ...
      bool ok = isBuy ? trade.Buy(lots,  sym, price, sl, tp, "Buy Karen Kijun")
                      : trade.Sell(lots, sym, price, sl, tp, "Sell Karen Kijun");
      if(!ok) break;
         ...
         if(st != 0)
         {
            ...
            if(filled > 0)
            {
               ...
               string msg = PCTime() + " | " + action + " " + syms[s] +
                            ...
                            " @ " + DoubleToString(lots, 2) + " (Karen Kijun)";
               if(tp > 0)
