//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA (H1 bias) — NEWS BLACKOUT (EXPERIMENT)|
//| EXPERIMENT — NOT DEPLOYED. Fork of ichimoku-h4-m1-vps-ea.mq5     |
//| (the live VPS build, bottom-up H1-bias stack) with the NEWS       |
//| BLACKOUT added: positions are flattened ahead of high-impact      |
//| events and entries are blocked until the post-event window        |
//| closes. The production VPS file is untouched.                     |
//|                                                                  |
//| NEWS BLACKOUT (REFINED, MT5 built-in Economic Calendar, default  |
//| ON): high-impact ("red folder") events on the symbol's currencies |
//| from the terminal's built-in calendar (no WebRequest, no DLL, no  |
//| scraping). Open positions on the affected symbol are closed       |
//| InpNewsBlockBeforeMin minutes before the release and no entry is  |
//| taken until InpNewsBlockAfterMin minutes after it. Medium impact  |
//| (orange) optional via InpNewsIncludeMedium; extra currencies via  |
//| InpNewsCurrencies (GOLDm# picks up USD through its profit         |
//| currency automatically). Fails OPEN with a one-shot warning when  |
//| the calendar is unreadable; the calendar is empty in the          |
//| Strategy Tester, so backtests trade as if no news existed.        |
//| VPS style kept: Print + SendNotification only (no Alert popups).  |
//|                                                                  |
//| Magic: 20260854 — fresh, distinct from every other build.         |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
...
//| Magic: 20260854 — fresh for this news-blackout fork, so it never  |
//|        touches positions opened by the live VPS build (20260850), |
//|        the desktop build (20260852) or any experiment.            |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
int MAGIC = 20260854;   // news-blackout fork (live VPS = 20260850, desktop = 20260852)
...
void OnTick()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      int    topDir  = 0;
      string topVia  = "--";
      if(SpreadOK(syms[s]) && TimeTradeServer() >= newsBlockUntil[s])
      {
