//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA — FIVE-BAND RISK LADDER               |
//| Magic 20260873. A fork of the LIVE VPS build                      |
//| (ichimoku-h4-m1-vps-ea.mq5, magic 20260858), which is left        |
//| untouched.                                                        |
//|                                                                  |
//| THE TRADING LOGIC IS THE LIVE BUILD'S, BYTE FOR BYTE. Five        |
//| tiers (M5, M15, M30, H1, H4), M1 as the start of every chain and  |
//| never a tier of its own, no M2 anywhere, H4 bias with the H1      |
//| stand-in, D1 filter on H4, kumo-touch exits, no profit target,    |
//| the 8xATR disaster stop and the robustness pack R2-R6. The ONLY   |
//| change in this file is the risk ladder below.                     |
//|                                                                  |
//| This build deliberately drops the M1 and M2 trading tiers, the    |
//| hard take profits and the tight hard stops explored in magics     |
//| 20260870 / 20260871 / 20260872 (notes sections 43, 44, 45). Those |
//| files are kept as the record; nothing from them survives here     |
//| except the band edges and percentages.                            |
//|                                                                  |
//|------------------------------------------------------------------|
//| THE RISK LADDER: THREE BANDS BECOME FIVE, AND 13000+ IS RAISED   |
//|                                                                  |
//|   band  equity          H4     total   $ at the band floor       |
//|   1     < 7000        20.0%   37.00%   $1,110 @ 3000            |
//|   2     7000-13000    10.0%   18.50%   $1,295 @ 7000            |
//|   3     13000-17000    7.0%   12.95%   $1,684 @ 13000           |
//|   4     17000-20000    4.0%    7.40%   $1,258 @ 17000           |
//|   5     20000+         2.0%    3.70%   $  740 @ 20000           |
//|                                                                  |
//| Bands 1 and 2 are the live build's, unchanged. H4 anchors each    |
//| band and every other tier holds the fixed fraction of it this     |
//| ladder has always used (M5/M15 0.05, M30 0.25, H1 0.5).           |
//|                                                                  |
//| WHY: the live build cut H4 from 10.0% to 2.0% at 13000 — a 5x     |
//| drop that outran the equity growth which triggered it. A 13k      |
//| account therefore risked FEWER DOLLARS ($448) than a 7k one       |
//| ($1,295): it grew into trading smaller.                           |
//|                                                                  |
//| THREE THINGS TO KNOW ABOUT THE NEW BANDS:                         |
//| * 13000 IS NO LONGER A DE-RISKING POINT — IT IS A STEP UP. The    |
//|   band-3 floor risks $1,684 against band 2's $1,295, so crossing  |
//|   13000 INCREASES money at risk by ~30%. De-risking now begins    |
//|   at 17000. Intended, but the opposite of what the live build     |
//|   does at this edge — the first thing to re-read if drawdown      |
//|   past 13k looks wrong.                                           |
//| * M30 in band 3 is 1.75%, against the live build's 0.20%. That    |
//|   band was the ONE place M30 broke the ladder's shape (0.10xH4    |
//|   where every other band uses 0.25xH4); restoring the shape means |
//|   M30 gains more here than its neighbours. Set                    |
//|   InpRiskPctM30_T3 = 0.7 to hold M30 at the old ratio.            |
//| * BAND 5 MATCHES the live build's old 13000+ risk (H4 2.0%)       |
//|   rather than going below it, so this ladder sits at or above     |
//|   the live one everywhere past 13000.                             |
//|                                                                  |
//| The % in the inputs is the SIZING basis (2xATR); the disaster     |
//| stop sits at 8xATR, so a full stop-out costs 4x the figure —      |
//| band 1 is 148% at the disaster stop, band 3 51.8%, band 5 14.8%.  |
//| OnInit prints the resolved ladder with totals.                    |
//|                                                                  |
//|------------------------------------------------------------------|
//| INHERITED FROM THE LIVE VPS BUILD, UNCHANGED:                    |
//+------------------------------------------------------------------+
...
//--- Band 1 — equity < InpRiskTier2At. Unchanged from the live build.
//--- H4 anchors each band; every other tier is the fixed fraction of it
//--- this ladder has always used (M5/M15 .05, M30 .25, H1 .5).
input double InpRiskPctM5       = 1.0;    // M5   — band 1 (equity < Tier2At)
...
//--- Band 2 — InpRiskTier2At .. InpRiskTier3At. Unchanged from the live build.
input double InpRiskPctM5_T2    = 0.5;    // M5   — band 2 (half regime)
...
//--- Band 3 — InpRiskTier3At .. InpRiskTier4At. THE CHANGE: the live build
//--- cut H4 to 2.0 here, a 5x drop that outran the equity growth triggering
//--- it, so a 13k account risked FEWER dollars ($448) than a 7k one
//--- ($1,295). At H4 7.0 the 13k floor risks $1,684 — 13000 is now a risk
//--- STEP UP and the de-risking begins at 17000 instead.
input double InpRiskPctM5_T3    = 0.35;   // M5   — band 3
input double InpRiskPctM15_T3   = 0.35;   // M15  — band 3
input double InpRiskPctM30_T3   = 1.75;   // M30  — band 3 (live build had 0.2 — see the header note on M30's ratio)
...
//--- Band 5 — InpRiskTier5At and above. New. H4 2.0 MATCHES the live
//--- build's old 13000+ figure rather than going below it, so this ladder
//--- sits at or above the live one everywhere past 13000.
...
int MAGIC = 20260873;   // live VPS build + the five-band risk ladder — nothing else changed
...
int OnInit()
{
   ...
   //--- multiple, so a stop-out costs about four times the figure. With five
   //--- bands instead of three, and the upper three deliberately raised,
   //--- stating the resolved ladder once at startup is cheaper than working
   //--- it back out of the input list.
   ...
   PrintFormat("   band  equity range                M5      M15     M30     H1      H4     TOTAL");
   PrintFormat("   1     below %-9.0f         %6.2f  %6.2f  %6.2f  %6.2f  %6.2f  %6.2f",
               InpRiskTier2At, InpRiskPctM5, InpRiskPctM15, InpRiskPctM30, InpRiskPctH1, InpRiskPctH4,
               InpRiskPctM5 + InpRiskPctM15 + InpRiskPctM30 + InpRiskPctH1 + InpRiskPctH4);
   PrintFormat("   2     %-6.0f .. %-9.0f    %6.2f  %6.2f  %6.2f  %6.2f  %6.2f  %6.2f",
               InpRiskTier2At, InpRiskTier3At, InpRiskPctM5_T2, InpRiskPctM15_T2, InpRiskPctM30_T2,
               InpRiskPctH1_T2, InpRiskPctH4_T2,
               InpRiskPctM5_T2 + InpRiskPctM15_T2 + InpRiskPctM30_T2 + InpRiskPctH1_T2 + InpRiskPctH4_T2);
   PrintFormat("   3     %-6.0f .. %-9.0f    %6.2f  %6.2f  %6.2f  %6.2f  %6.2f  %6.2f",
               InpRiskTier3At, InpRiskTier4At, InpRiskPctM5_T3, InpRiskPctM15_T3, InpRiskPctM30_T3,
               InpRiskPctH1_T3, InpRiskPctH4_T3,
               InpRiskPctM5_T3 + InpRiskPctM15_T3 + InpRiskPctM30_T3 + InpRiskPctH1_T3 + InpRiskPctH4_T3);
   PrintFormat("   4     %-6.0f .. %-9.0f    %6.2f  %6.2f  %6.2f  %6.2f  %6.2f  %6.2f",
               InpRiskTier4At, InpRiskTier5At, InpRiskPctM5_T4, InpRiskPctM15_T4, InpRiskPctM30_T4,
               InpRiskPctH1_T4, InpRiskPctH4_T4,
               InpRiskPctM5_T4 + InpRiskPctM15_T4 + InpRiskPctM30_T4 + InpRiskPctH1_T4 + InpRiskPctH4_T4);
   PrintFormat("   5     %-6.0f and above       %6.2f  %6.2f  %6.2f  %6.2f  %6.2f  %6.2f",
               InpRiskTier5At, InpRiskPctM5_T5, InpRiskPctM15_T5, InpRiskPctM30_T5,
               InpRiskPctH1_T5, InpRiskPctH4_T5,
               InpRiskPctM5_T5 + InpRiskPctM15_T5 + InpRiskPctM30_T5 + InpRiskPctH1_T5 + InpRiskPctH4_T5);
   PrintFormat("   ACTIVE at equity %.2f -> M5 %.2f%% (~%.2f%% at stop), M30 %.2f%% (~%.2f%%), H4 %.2f%% (~%.2f%%)",
               AccountInfoDouble(ACCOUNT_EQUITY),
               LevelRiskPct(0), LevelRiskPct(0) * (InpDisasterATRMult / InpRiskATRMult),
               LevelRiskPct(2), LevelRiskPct(2) * (InpDisasterATRMult / InpRiskATRMult),
               LevelRiskPct(4), LevelRiskPct(4) * (InpDisasterATRMult / InpRiskATRMult));
...
double LevelRiskPct(int lvl)
{
   ...
   switch(lvl)
   {
      case 0:  switch(band){ case 5: return InpRiskPctM5_T5;  case 4: return InpRiskPctM5_T4;
                             case 3: return InpRiskPctM5_T3;  case 2: return InpRiskPctM5_T2;  } return InpRiskPctM5;
      case 1:  switch(band){ case 5: return InpRiskPctM15_T5; case 4: return InpRiskPctM15_T4;
                             case 3: return InpRiskPctM15_T3; case 2: return InpRiskPctM15_T2; } return InpRiskPctM15;
      case 2:  switch(band){ case 5: return InpRiskPctM30_T5; case 4: return InpRiskPctM30_T4;
                             case 3: return InpRiskPctM30_T3; case 2: return InpRiskPctM30_T2; } return InpRiskPctM30;
      case 3:  switch(band){ case 5: return InpRiskPctH1_T5;  case 4: return InpRiskPctH1_T4;
                             case 3: return InpRiskPctH1_T3;  case 2: return InpRiskPctH1_T2;  } return InpRiskPctH1;
      case 4:  switch(band){ case 5: return InpRiskPctH4_T5;  case 4: return InpRiskPctH4_T4;
