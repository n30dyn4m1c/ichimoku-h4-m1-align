//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA — H1 BIAS, FIXED 0.1 LOT (TEST)       |
//| Fork of experimental-bottomup-stack-h1-bias-ea.mq5: same entries, |
//| same exits, same H1 stand-in bias — but the whole equity-% risk   |
//| regime is stripped out and EVERY trade opens at a flat 0.10 lot.  |
//| TESTING ONLY. With one constant size, every tier and every bias   |
//| path contributes to the P/L on equal footing, so a tester run     |
//| measures the SIGNAL instead of the sizing table. This is not an   |
//| account-scaling build and must not be run as one.                 |
...
//|        a time (the highest aligned tier). Sizing is FLAT: every   |
//|        trade opens at InpTestLots (0.10) whatever the tier, the   |
//|        bias, the equity or the ATR. No equity %, no 3-tier regime,|
//|        no ATR-scaled sizing, no multipliers, no compounding. The  |
//|        lot is still snapped to the symbol's volume step, clamped  |
//|        to its min/max and capped to free margin so the order fills|
//|        — each of those is logged when it bites, so a run at a size|
//|        other than 0.10 is never silent. Still NO entry stop loss; |
//|        the BE/chandelier layer is the only protection once green. |
//| Magic: 20260851 — fresh, so this test build runs side by side with|
//|        the 20260850 H1-bias build, the 20260848 snapshot and the  |
//|        live VPS builds                                            |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
input group  "Position Sizing (TEST BUILD — flat lot, no risk regime)"
input double InpTestLots = 0.10;   // Every trade opens at this lot size, whatever the tier / bias / equity
...
int MAGIC = 20260851;   // fresh — flat-lot test build (20260850 is the H1-bias build it forks, 20260848 the snapshot)
...
//==============================================================
// Position Sizing (TEST BUILD) — flat lot, no risk regime.
//
// The parent build sized every entry as a % of live equity from a
// tier x equity-regime table, so an H4-tier trade could carry 200x
// the exposure of an M5 one and the P/L was dominated by the sizing
// table rather than by the entries. Here every trade — every tier,
// every bias path, at any equity — opens at InpTestLots, so a
// tester run measures the SIGNAL: trade count, win rate and average
// R per tier and per bias are directly comparable across the stack.
//
// The only adjustments left are the ones the broker forces: the lot
// is snapped to the symbol's volume step and clamped to its min/max.
// Both are logged, so a run that silently traded a size other than
// InpTestLots cannot be mistaken for a clean one.
//==============================================================

double FlatLots(int s)
{
   double lots    = InpTestLots;
   ...
   if(lotMin  > 0) lots = MathMax(lotMin, lots);
   if(lotMax  > 0) lots = MathMin(lotMax, lots);

   if(MathAbs(lots - InpTestLots) > 1e-8)
      Print(PCTime() + " | " + syms[s] + " test lot " + DoubleToString(InpTestLots, 2) +
            " adjusted to " + DoubleToString(lots, 2) +
            " by the symbol volume step/min/max — results are NOT at the flat test size");
...
void CapLotsToMargin(string sym, bool isBuy, double &lots)
{
   ...
   if(lots > maxLots)
   {
      // Test build: never shrink the flat lot silently — a margin-capped
      // entry is not a result at InpTestLots and has to be visible.
      Print(PCTime() + " | " + sym + " test lot " + DoubleToString(lots, 2) +
            " capped to " + DoubleToString(maxLots, 2) +
            " by free margin — results are NOT at the flat test size");
      lots = maxLots;
   }
}
