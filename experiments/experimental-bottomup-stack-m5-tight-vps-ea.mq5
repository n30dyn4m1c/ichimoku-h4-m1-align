//+------------------------------------------------------------------+
//| EXPERIMENT — M5 TIER TIGHTENED (magic 20260875, notes §48)       |
//| Forked from the scalp-capture experiment (§47), which is the     |
//| live VPS build (20260858, untouched) plus a per-tier report.     |
//| Scalp capture is OFF by default here, so with every M5 filter    |
//| below switched off this file trades exactly like the live build. |
//|                                                                  |
//| Why: the §47 backtest (GOLDm#, Jan-Sep 2026, real ticks) showed  |
//| the M5 tier is 47% of all trades for ~6% of the profit (PF 1.10) |
//| — the most trades, the thinnest edge, the most spread paid.      |
//|                                                                  |
//| Filters that apply ONLY to the M5 tier (M15+ untouched):         |
//|   InpM5Enabled        off = no M5 trades at all (the control)    |
//|   InpM5NeedH4         M5 needs the H4 bias itself — no H1        |
//|                       stand-in                                   |
//|   InpM5NeedH1         H1 must be aligned with the trade too      |
//|   InpM5CloudFull      M5 cloud twisted the trade's way now AND   |
//|                       at the far end (the M1 rule, applied to M5)|
//|   InpM5MaxCloudATR    entry no further than this x ATR(M5) from  |
//|                       the M5 cloud edge — the kumo-touch exit is |
//|                       the stop, so this caps the loss per trade  |
//|   InpM5MaxSpread      tighter spread cap for M5 entries          |
//| Default: InpM5NeedH4 = true — the one filter that raised the M5  |
//| tier's profit factor in both 2025 and 2026 (1.12->1.22,          |
//| 1.10->1.29). The rest stay off; the distance cap made M5 worse.  |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
...
// Highest tier the scalp-capture layer manages (H1/H4 already trail from
...
input bool            InpScalpCapture  = false;     // §47 verdict: not worth it — off here
...
input group  "M5 Tier Tightening (EXPERIMENT — M5 tier only)"
input bool   InpM5Enabled      = true;   // false = the M5 tier never opens (control run)
input bool   InpM5NeedH4       = true;   // M5 needs the H4 bias itself (no H1 stand-in) — the §48 winner
input bool   InpM5NeedH1       = false;  // H1 must also be aligned with the M5 trade
input bool   InpM5CloudFull    = false;  // M5 cloud must agree now AND at the far end (like M1)
...
#define MFE_BUCKETS 5                    // MFE in ATR: <0.5, 0.5-1, 1-2, 2-3, 3+
int      stN[LEVELS], stWin[LEVELS], stGive[LEVELS], stScalp[LEVELS];
int      stMfe[LEVELS][MFE_BUCKETS];
double   stNet[LEVELS], stGrossW[LEVELS], stGrossL[LEVELS];
...
int MAGIC = 20260875;   // M5-tight experiment — never manages the live build's (20260858) positions
...
int OnInit()
{
   ...
   for(int l = 0; l < LEVELS; l++)
   {
      stN[l] = 0; stWin[l] = 0; stGive[l] = 0; stScalp[l] = 0;
      stNet[l] = 0.0; stGrossW[l] = 0.0; stGrossL[l] = 0.0;
      for(int b = 0; b < MFE_BUCKETS; b++) stMfe[l][b] = 0;
   }

   if(InpScalpCapture)
      Print("Scalp capture ON: M5.." + tfName[(int)InpScalpMaxTier + 1] +
            " bank " + DoubleToString(InpScalpClosePct, 0) + "% at +" +
            DoubleToString(InpScalpTP1ATR, 2) + " ATR, runner locked at +" +
            DoubleToString(InpScalpLockATR, 2) + " ATR, trail " +
            DoubleToString(InpScalpTrailATR, 2) + " ATR");
   else
      Print("Scalp capture OFF: live build's exits (report still collected)");
...
{
   // Positions still open at the end of a test are closed by the tester
   // after OnDeinit, so they are missing from the report — it says so.
   PrintTierReport();
...
//==============================================================
// Per-tier report (EXPERIMENT). When a level's trade is gone,
// its deals are summed by position identifier — partial scalp
// closes included — and the trade is filed under its tier with
// its MFE in ATR-at-entry units. A GIVE-BACK is a trade that was
// +1 ATR in profit at some point and still closed at or below 0:
// the scalp the exit logic let slip.
//==============================================================

void RecordClosedTrade(int s, int l)
{
   ulong posId = posIdOf[s][l];
   if(posId == 0) return;
   if(PositionSelectByTicket(posId)) return;   // still open — nothing to file yet
   ...
   for(int d = 0; d < HistoryDealsTotal(); d++)
   {
      ...
      net += HistoryDealGetDouble(dt, DEAL_PROFIT) + HistoryDealGetDouble(dt, DEAL_SWAP) +
             HistoryDealGetDouble(dt, DEAL_COMMISSION);
      long entry = HistoryDealGetInteger(dt, DEAL_ENTRY);
      if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY) outs++;
   }
   if(outs == 0) return;                       // closing deal not in history yet — retry later
   posIdOf[s][l] = 0;

   stN[l]++;
   stNet[l] += net;
   if(net > 0) { stWin[l]++; stGrossW[l] += net; }
   else          stGrossL[l] -= net;
   if(outs > 1) stScalp[l]++;                  // a partial close happened on this trade

   double atrE = atrEntry[s][l];
   if(atrE <= 0 || entryPrice[s][l] <= 0) return;
   double mfe = (posDir[s][l] == -1) ? (entryPrice[s][l] - peakLow[s][l]) / atrE
                                     : (peakHigh[s][l] - entryPrice[s][l]) / atrE;
   int b = (mfe < 0.5) ? 0 : (mfe < 1.0) ? 1 : (mfe < 2.0) ? 2 : (mfe < 3.0) ? 3 : 4;
   stMfe[l][b]++;
   if(mfe >= 1.0 && net <= 0) stGive[l]++;
}

void PrintTierReport()
{
   Print("==== Per-tier report (magic " + IntegerToString(MAGIC) + ", scalp capture " +
         (InpScalpCapture ? "ON" : "OFF") + ") — closed trades only ====");
   Print("tier | trades | win% | net | PF | avg win | avg loss | MFE<0.5 / 0.5-1 / 1-2 / 2-3 / 3+ ATR | give-backs | scalps banked");
   for(int l = 0; l < LEVELS; l++)
   {
      if(stN[l] == 0) { Print(tfName[l + 1] + " | 0"); continue; }
      int    losses = stN[l] - stWin[l];
      double pf     = (stGrossL[l] > 0) ? stGrossW[l] / stGrossL[l] : 0.0;
      string pfs    = (stGrossL[l] > 0) ? DoubleToString(pf, 2) : "inf";
      double aw     = (stWin[l] > 0) ? stGrossW[l] / stWin[l] : 0.0;
      double al     = (losses > 0)   ? stGrossL[l] / losses   : 0.0;
      Print(tfName[l + 1] + " | " + IntegerToString(stN[l]) + " | " +
            DoubleToString(100.0 * stWin[l] / stN[l], 1) + " | " +
            DoubleToString(stNet[l], 2) + " | " + pfs + " | " +
            DoubleToString(aw, 2) + " | " + DoubleToString(al, 2) + " | " +
            IntegerToString(stMfe[l][0]) + " / " + IntegerToString(stMfe[l][1]) + " / " +
            IntegerToString(stMfe[l][2]) + " / " + IntegerToString(stMfe[l][3]) + " / " +
            IntegerToString(stMfe[l][4]) + " | " + IntegerToString(stGive[l]) + " | " +
            IntegerToString(stScalp[l]));
   }
...
void SyncStateFromPositions()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      for(int s = 0; s < symsCount; s++)
      {
         ...
         bool   rebuilt = (posIdOf[s][lvlMatch] != posId);
         posIdOf[s][lvlMatch] = posId;
         posDir[s][lvlMatch]  = dir;
         ...
         // Scalp capture after a restart: the entry ATR is approximated by
         // the current one, and a position smaller than its entry volume has
         // already banked its scalp. History is read last — it must not
         // disturb the position fields read above.
         if(rebuilt)
         {
            double a[1];
            if(atrEntry[s][lvlMatch] <= 0.0 &&
               CopyBuffer(atr[s][lvlMatch], 0, 1, 1, a) > 0 && a[0] > 0)
               atrEntry[s][lvlMatch] = a[0];
            ...
            if(HistorySelectByPosition(posId))
               for(int d = 0; d < HistoryDealsTotal(); d++)
               {
                  ...
                  if(HistoryDealGetInteger(dt, DEAL_ENTRY) == DEAL_ENTRY_IN)
                     inVol += HistoryDealGetDouble(dt, DEAL_VOLUME);
               }
            double step = SymbolInfoDouble(sym, SYMBOL_VOLUME_STEP);
            if(inVol > 0 && posVol < inVol - 0.5 * step) scalpDone[s][lvlMatch] = true;
         }
         break;
      }
   }
...
//==============================================================
// M5 Tier Gate (EXPERIMENT): extra conditions on the M5 tier
// only, each switchable. 'via' is the bias EntryBiasOK granted.
//==============================================================

bool M5TierOK(int s, int dir, string via)
{
   if(!InpM5Enabled) return false;
   if(InpM5NeedH4 && via != "H4") return false;
   if(InpM5NeedH1 && CheckAlign(s, IDX_H1) != dir) return false;
   if(InpM5CloudFull && !CloudBiasOK(s, 1, dir)) return false;
   if(InpM5MaxSpread > 0 && SymbolInfoInteger(syms[s], SYMBOL_SPREAD) > InpM5MaxSpread) return false;

   if(InpM5MaxCloudATR > 0)
   {
      // The kumo-touch exit is where a loser closes, so the distance from
      // the entry to the M5 cloud edge is the trade's real risk.
      double a[1], senA[1], senB[1];
      if(CopyBuffer(atr[s][0], 0, 1, 1, a) <= 0 || a[0] <= 0) return false;
      if(CopyBuffer(ich[s][1], 2, 1, 1, senA) <= 0) return false;
      if(CopyBuffer(ich[s][1], 3, 1, 1, senB) <= 0) return false;
      double px   = (dir == 1) ? SymbolInfoDouble(syms[s], SYMBOL_ASK)
                               : SymbolInfoDouble(syms[s], SYMBOL_BID);
      double dist = (dir == 1) ? px - MathMax(senA[0], senB[0])
                               : MathMin(senA[0], senB[0]) - px;
      if(dist > InpM5MaxCloudATR * a[0]) return false;
   }
   return true;
}
...
bool OpenLevel(int s, int lvl, int dir, double lots, string via)
{
   ...
   // Report: a trade on this level that closed earlier this minute must be
   // filed before its references are overwritten by the new one.
   RecordClosedTrade(s, lvl);
   ...
   if(ok)
   {
      ...
      // The position identifier is the ticket of the order that opened it
      posIdOf[s][lvl]    = trade.ResultOrder();
...
void ManageLevelProtection(int s, int lvl)
{
   ...
   // SCALP CAPTURE (EXPERIMENT): at +InpScalpTP1ATR x ATR(entry) bank
   // InpScalpClosePct % and lock the runner at entry + InpScalpLockATR x
   // ATR. One-shot per trade. Runs before BE so the lock, which is tighter
   // than BE's entry + cover, is what the runner carries.
   bool scalpTier = InpScalpCapture && lvl <= (int)InpScalpMaxTier;
   if(scalpTier && !scalpDone[s][lvl])
   {
      double atrE = (atrEntry[s][lvl] > 0) ? atrEntry[s][lvl] : atrVal;
      bool hit = isLong ? (bid >= entryPrice[s][lvl] + InpScalpTP1ATR * atrE)
   ...
   bool armed = runner ||
                (isLong ? (bid >= entryPrice[s][lvl] + armATR * atrVal)
                        : (ask <= entryPrice[s][lvl] - armATR * atrVal));
   if(armed)
   {
      double slNew = isLong ? peakHigh[s][lvl] - trailATR * atrVal
                            : peakLow[s][lvl] + trailATR * atrVal;
      slNew = NormalizeDouble(slNew, digits);
