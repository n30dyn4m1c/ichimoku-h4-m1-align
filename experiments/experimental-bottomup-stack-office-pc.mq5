//+------------------------------------------------------------------+
//| Ichimoku Bottom-Up Stack EA — PARTIAL TP (EXPERIMENT)             |
//| Fork of experimental-bottomup-stack-ea-very-profitable.mq5 with   |
//| ONE change: the H1 and H4 tiers scale out at a fixed R multiple.  |
//| Everything else — entry, kumo-touch exit, BE, chandelier trail,   |
//| risk regime — is byte-identical to that baseline, so a backtest   |
//| against it isolates the partial-TP effect alone.                  |
//|                                                                   |
...
//| PARTIAL TP (the only change vs the baseline): the H1 and H4       |
//|        tiers bank a fraction of the position at a fixed multiple  |
//|        of R, where R is the SAME reference distance the position  |
//|        was sized on — ATR(tier TF) x InpRiskATRMult, captured at  |
//|        entry so the target does not drift as ATR changes. With    |
//|        the defaults (InpRiskATRMult 2.0, trigger 2.0 R) that is   |
//|        4 x ATR(tier TF) of profit, where half the position is     |
//|        closed and the remainder runs on the untouched kumo exit   |
//|        + trail. BE (0.5 x ATR on H1/H4) always arms long before   |
//|        the partial fires, so the runner is already risk-free at   |
//|        that point. One-shot per trade; the lower tiers            |
//|        (M5/M15/M30) are deliberately untouched.                   |
...
//|        compounding. Risk percentages are UNCHANGED from the       |
//|        baseline on purpose — partial TP is the single variable    |
//|        under test.                                                |
//| Magic: 20260849 — distinct from the baseline (20260848) and the   |
//|        live VPS builds, so both can run side by side              |
//| Author: Neo Malesa                                               |
//+------------------------------------------------------------------+
#property strict
...
input double InpRiskATRMult     = 2.0;    // Reference stop distance = ATR(level TF) x this (risk sizing basis, and 1 R for partial TP)
...
input group  "Partial Take Profit (H1 and H4 tiers only)"
input bool   InpPartialTPEnabled = true;   // Bank a fraction of H1/H4 positions at a fixed R multiple
input double InpPartialR_H4      = 2.0;    // H4 tier: bank when profit >= this x R (R = ATR(H4) x InpRiskATRMult; 0 = off)
input double InpPartialPct_H4    = 50.0;   // H4 tier: % of the position to close at the trigger
input double InpPartialR_H1      = 2.0;    // H1 tier: bank when profit >= this x R (R = ATR(H1) x InpRiskATRMult; 0 = off)
input double InpPartialPct_H1    = 50.0;   // H1 tier: % of the position to close at the trigger
...
#define LVL_H1   3      // level index of the H1 tier (partial TP applies from here up)
#define LVL_H4   4      // level index of the H4 tier
...
double   riskDist[MAX_SYMS][LEVELS];     // 1 R = ATR(level TF) x InpRiskATRMult, frozen at entry
bool     partialDone[MAX_SYMS][LEVELS];  // partial TP already banked on this trade (one-shot)

int MAGIC = 20260849;   // partial-TP fork (20260848 is the bottom-up baseline, 20260846/47 the live VPS builds)
...
// The partial-TP memory cannot be recovered from the position (a banked
// tranche leaves no marker), so a restart mid-trade clears the one-shot
// flag: a trade already ABOVE its trigger will bank a second tranche on
// the next M1 bar.
      ...
      for(int s = 0; s < symsCount; s++)
      {
         ...
         for(int l = 0; l < LEVELS; l++)
         {
            if(comm == LevelComment(l, 1) || comm == LevelComment(l, -1))
            {
               ...
               // R was frozen at entry and is gone after a restart — rebuild it
               // from the current ATR so the partial trigger still has a scale
               if(riskDist[s][l] == 0.0)
               {
                  double ar[1];
                  if(CopyBuffer(atr[s][l], 0, 1, 1, ar) > 0 && ar[0] > 0)
                     riskDist[s][l] = ar[0] * InpRiskATRMult;
               }
               break;
            }
         }
...
// Round a volume DOWN to the symbol's lot step and normalize it to the
// step's decimal precision, so a partial close never trips "invalid
// volume" on a float-rounding remainder.
...
//
// That same reference distance is 1 R for the partial-TP trigger,
// so "bank at 2 R" means "bank at twice the distance the position
// was sized against".
//==============================================================
...
bool OpenLevel(int s, int lvl, int dir, double lots)
{
   ...
   if(ok)
   {
      ...
      peakLow[s][lvl]    = price;
      beMoved[s][lvl]    = false;
      partialDone[s][lvl] = false;

      // Freeze 1 R at entry — the same distance the position was sized
      // against — so the partial target does not drift when ATR changes
      riskDist[s][lvl] = 0.0;
      double a[1];
      if(CopyBuffer(atr[s][lvl], 0, 1, 1, a) > 0 && a[0] > 0)
         riskDist[s][lvl] = a[0] * InpRiskATRMult;
...
//==============================================================
// Partial Take Profit — H1 and H4 tiers only.
//
// The H1 and H4 tiers carry by far the largest risk percentages
// (10% and 20% of equity in tier 1), and their kumo exit sits the
// furthest from entry, so they are where a reversal costs the
// most. Banking a fraction of the position at a fixed multiple of
// R turns part of that exposure into realised profit while leaving
// the remainder to run on the untouched kumo exit and chandelier
// trail — cutting the give-back without capping the winner.
//
// R is the SAME reference distance the position was sized against
// (ATR(tier TF) x InpRiskATRMult), frozen at entry so the target
// does not drift as ATR changes. With the defaults that puts the
// H4 trigger at 2 R = 4 x ATR(H4) of profit.
//
// BE (0.5 x ATR on H1/H4) always arms long before the partial
// fires, so by the time a tranche is banked the runner is already
// protected at break even — the banked profit cannot be given back.
//
// One-shot per trade. If the position cannot be split into two
// legally tradable volumes (either side below the broker minimum),
// the attempt is marked done so it is not retried every bar.
//==============================================================

void ManagePartialTP(int s, int lvl)
{
   if(!InpPartialTPEnabled) return;
   if(lvl < LVL_H1) return;                  // H1 and H4 tiers only
   if(partialDone[s][lvl]) return;

   int dir = state[s][lvl];
   if(dir == 0) return;

   double R = riskDist[s][lvl];
   if(R <= 0) return;                        // no scale available — skip

   double trigR = (lvl == LVL_H4) ? InpPartialR_H4   : InpPartialR_H1;
   double pct   = (lvl == LVL_H4) ? InpPartialPct_H4 : InpPartialPct_H1;
   if(trigR <= 0 || pct <= 0) return;        // tier disabled

   bool   isLong = (dir == 1);
   double bid    = SymbolInfoDouble(syms[s], SYMBOL_BID);
   double ask    = SymbolInfoDouble(syms[s], SYMBOL_ASK);

   bool hit = isLong ? (bid >= entryPrice[s][lvl] + trigR * R)
                     : (ask <= entryPrice[s][lvl] - trigR * R);
   if(!hit) return;

   ulong ticket;
   if(!LevelTicket(s, lvl, ticket)) return;  // selects the position
   double vol = PositionGetDouble(POSITION_VOLUME);
   if(vol <= 0) return;

   double lotMin   = SymbolInfoDouble(syms[s], SYMBOL_VOLUME_MIN);
   double closeVol = NormalizeVolume(syms[s], vol * (pct / 100.0));
   double restVol  = NormalizeVolume(syms[s], vol - closeVol);

   // Both legs of the split must be tradable volumes, otherwise the broker
   // rejects the request — leave the position whole and stop retrying.
   if(closeVol < lotMin || restVol < lotMin)
   {
      partialDone[s][lvl] = true;
      Print(PCTime() + " | " + syms[s] + " " + tfName[lvl + 1] +
            " partial TP skipped — volume " + DoubleToString(vol, 2) +
            " cannot split above the " + DoubleToString(lotMin, 2) + " minimum");
      return;
   }
   ...
   {
      partialDone[s][lvl] = true;
      string msg = PCTime() + " | Partial TP " + syms[s] + " " +
                   (isLong ? "Long " : "Short ") + tfName[lvl + 1] + " — banked " +
                   DoubleToString(closeVol, 2) + " of " + DoubleToString(vol, 2) +
                   " at " + DoubleToString(trigR, 1) + "R, " +
                   DoubleToString(restVol, 2) + " runs on";
      Print(msg); SendNotification(msg);
   }
   else
   {
      Print(PCTime() + " | " + syms[s] + " " + tfName[lvl + 1] +
            " partial TP failed, retcode " + IntegerToString(trade.ResultRetcode()));
   }
}
...
void OnTick()
{
   ...
   for(int s = 0; s < symsCount; s++)
   {
      ...
      for(int l = 0; l < LEVELS; l++)
      {
         ...
         // Partial TP: bank a tranche of the big H1/H4 positions at a
         // fixed R multiple, leaving the rest to run
         if(state[s][l] != 0) ManagePartialTP(s, l);
